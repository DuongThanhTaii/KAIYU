...
function QuizContent() {
  ...
  const handleCreateManual = async () => {
    ...
    const proceedCreate = async () => {
      ...
      try {
        ...
        const errorMsg = extractErrorMessage(err, "Không thể tạo tự động.");
    ...
    if (quiz) {
      setConfirmAction({
        isOpen: true,
        message:
          "Bạn đã có bài tập cho video này. Việc tạo mới thủ công sẽ xóa toàn bộ câu hỏi hiện tại. Bạn có chắc chắn muốn tiếp tục?",
  ...
  const handleGenerate = async () => {
    ...
    if (quiz) {
      setConfirmAction({
        isOpen: true,
        message:
          "Hệ thống sẽ dùng AI để phân tích lại toàn bộ phụ đề và tạo bài tập mới. TOÀN BỘ CÂU HỎI CŨ SẼ BỊ XÓA (nếu AI tạo thành công). Bạn có chắc chắn muốn tiếp tục?",
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      const newQuestion = await quizzesApi.addQuestion(quiz.id, {
        ...
        blankPosition: questionForm.sentenceHanzi.indexOf(
          questionForm.blankWord,
        ),
    ...
    if (!sentence.includes(blankWord)) {
      return (
        <p className="text-red-400 text-sm">
          ⚠️ Không tìm thấy &quot;{blankWord}&quot; trong câu. Vui lòng kiểm tra
          lại.
        </p>
      );
    }
      ...
      <>
        ...
        {multipleOccurrences && (
          <p className="text-amber-400 text-xs mt-2">
            💡 Từ &quot;{blankWord}&quot; xuất hiện nhiều lần - tất cả sẽ thành
            chỗ trống.
          </p>
        )}
      </>
  ...
  const renderInteractiveSentence = (sentence: string) => {
    if (!sentence)
      return <p className="text-text-secondary">Nhập câu tiếng Trung ở trên</p>;
        ...
        {sentence.split("").map((char, index) => {
          ...
          return (
            <button
              ...
              className={`px-1.5 py-1 rounded text-xl font-chinese transition-all ${
                isSelected
    ...
    <AdminLayout>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          {quiz && !quiz.isPublished && (
            <Button
              ...
              variant="primary"
              size="sm"
              className="flex-1 sm:flex-none whitespace-nowrap h-9 sm:h-10 text-xs sm:text-sm"
            >
      ...
      {!loading && !quiz && (
        <div className="bg-surface-dark rounded-2xl border border-border-color p-8 text-center">
          <Icon
            name="quiz"
            className="text-6xl text-text-secondary mx-auto mb-4"
          />
          <h2 className="text-xl font-bold text-white mb-2">Chưa có bài tập</h2>
          <p className="text-text-secondary mb-6">
            Sử dụng các nút phía trên để tạo bài tập thủ công hoặc tự động bằng
            trí tuệ nhân tạo.
          </p>
        </div>
      )}
                    ...
                    <div className="flex-1 w-full">
                      ...
                      <div className="flex flex-wrap gap-2 mt-4">
                        {question.options.map((option, optIdx) => (
                          <span
                            key={optIdx}
                            className={`px-3 py-1.5 rounded-lg text-sm sm:text-base font-chinese ${
                              option === question.blankWord
        ...
        footer={
          <>
            ...
            <button
              ...
              disabled={
                savingQuestion ||
                !questionForm.sentenceHanzi ||
                !questionForm.blankWord ||
                !questionForm.sentenceHanzi.includes(questionForm.blankWord)
              }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-text-secondary mt-1">
              👆 Click vào ký tự để chọn. Click ký tự đầu rồi click ký tự cuối
              để chọn nhiều ký tự.
            </p>
            {questionForm.blankWord && (
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setQuestionForm({ ...questionForm, meaningVi: e.target.value })
              }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setQuestionForm({ ...questionForm, blankWord: e.target.value })
              }
              className="w-full px-4 py-3 bg-background-dark border border-border-color rounded-xl text-white placeholder-text-secondary focus:outline-none focus:border-amber-500 transition-colors font-chinese"
              placeholder="VD: 高兴"
            />
            <p className="text-xs text-text-secondary mt-1">
              Từ này phải có trong câu ở trên
            </p>
          </div>
