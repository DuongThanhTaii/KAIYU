        ...
        {parts.map((part, idx) => {
          ...
          if (colonIndex > 0) {
            ...
            return (
              <div
                key={idx}
                className={`flex items-start ${spacingClass} leading-tight`}
              >
                <div className={containerClass}>
                  <span
                    className={`${colorClass} font-black mr-1.5 uppercase tracking-wide ${posClass}`}
                  >
                    {pos}:
                  </span>
  ...
  if (match) {
    ...
    return (
      <div className={`${containerClass} py-0.5`}>
        <span
          className={`${colorClass} font-black mr-2 uppercase tracking-wide ${posClass}`}
        >
          {pos}:
        </span>
        <span className="text-text-base font-medium">{meaning}</span>
...
export default function AdminVocabularyPage() {
  ...
  const [duplicateConfirmImport, setDuplicateConfirmImport] = useState<{
    duplicates: string[];
    total: number;
  } | null>(null);
  ...
  useEffect(() => {
    if (searchQuery && vocabulary.length > 0) {
      ...
      const firstMatch = vocabulary.find(
        (item) =>
      ...
      if (result && result.found) {
        ...
        if (result.meaningVi || result.meaningEn) {
          ...
          if (result.partOfSpeech)
            newMeanings[0].partOfSpeech = result.partOfSpeech;
  ...
  const handleOpenEdit = (vocab: Vocabulary) => {
    ...
    setFormData({
      ...
      strokeCount:
        vocab.strokeCount !== undefined ? String(vocab.strokeCount) : "",
      hskLevel: vocab.hskLevel,
      tags: Array.isArray(vocab.tags)
        ? vocab.tags.join(", ")
        : vocab.tags || "",
    ...
    const vocabWithMeanings = vocab as Vocabulary & {
      meanings?: MeaningVariant[];
    };
    const vocabMeanings = vocabWithMeanings.meanings;
    if (
      vocabMeanings &&
      Array.isArray(vocabMeanings) &&
      vocabMeanings.length > 0
    ) {
      setMeaningEntries(
        vocabMeanings.map((m: MeaningVariant) => ({
          partOfSpeech: (m.partOfSpeech || "").toLowerCase(),
          pinyin: m.pinyin || "",
          meanings: Array.isArray(m.meanings)
            ? m.meanings.join("; ")
            : m.meanings || "",
        })),
      );
    ...
    if (
      vocabExamples &&
      Array.isArray(vocabExamples) &&
      vocabExamples.length > 0
    ) {
      setExampleEntries(
        vocabExamples.map((e: VocabExampleItem) => ({
          chinese: e.chinese || "",
          pinyin: e.pinyin || "",
    ...
    if (vocabSynonyms && Array.isArray(vocabSynonyms)) {
      setSynonyms(
        vocabSynonyms
          .map((s: VocabLexicalItem) => `${s.hanzi}:${s.pinyin}:${s.meaningVi}`)
          .join(", "),
      );
    }
    ...
    if (vocabAntonyms && Array.isArray(vocabAntonyms)) {
      setAntonyms(
        vocabAntonyms
          .map((a: VocabLexicalItem) => `${a.hanzi}:${a.pinyin}:${a.meaningVi}`)
          .join(", "),
      );
    }
    setShowModal(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const primaryMeaning =
        meanings[0]?.meanings?.join("; ") || formData.meaningVi;
      ...
      if (errorMessage.includes("đã tồn tại")) {
        ...
        const existingVocab = vocabulary.find(
          (v) => v.hanzi === formData.hanzi,
        );
        if (existingVocab) {
  ...
  const getHskLevelFromSheetName = (name: string): number => {
    ...
    if (trimmed.includes("ngoài HSK") || trimmed.includes("Từ vựng ngoài"))
      return 7;
    if (trimmed.includes("không chia") || trimmed.includes("Không chia cấp"))
      return 0;
    return 1; // default
  };
  ...
  const parseSheetRow = (
    ...
    Object.keys(row).forEach((k) => {
      ...
      if (
        ...
        key === "hanzi" ||
        key === "汉字" ||
        key.includes("chữ hán")
      ) {
        hanzi = val;
      }
      // Pinyin: "pinyin", "拼音", "phiên âm ví dụ" is NOT pinyin of the word
      else if (
        (key === "pinyin" || key.startsWith("pinyin")) &&
        !key.includes("phiên âm")
      ) {
        pinyin = val;
      }
      // Part of speech: "từ loại", "partofspeech"
      else if (
        key.includes("từ loại") ||
        key === "partofspeech" ||
        key === "pos"
      ) {
        partOfSpeech = val;
      ...
      // IMPORTANT: Must check BEFORE 'đồng nghĩa' and 'trái nghĩa' to avoid false match
        ...
        (key === "nghĩa" ||
          key === "nghia" ||
          key === "meaningvi" ||
          key.includes("ý nghĩa")) &&
        !key.includes("đồng nghĩa") &&
        !key.includes("cận nghĩa") &&
        !key.includes("trái nghĩa")
      ) {
        meaningVi = val;
      }
      ...
      // Example Pinyin: "phiên âm" (for example sentence)
        ...
        key.includes("phiên âm") ||
        (key.startsWith("pinyin") && key.includes("phiên âm"))
      ) {
        examplePy = val;
      }
    ...
    if (unmatchedKeys.length > 0 && hanzi) {
      console.debug(
        `[parseSheetRow] hanzi='${hanzi}': unmatched headers: [${unmatchedKeys.join(", ")}]`,
      );
    }
      ...
      reader.onload = (event) => {
        try {
          ...
          for (const sheetName of workbook.SheetNames) {
            if (
              importTargetSheet !== "all" &&
              sheetName.trim() !== importTargetSheet
            )
              continue;
          ...
          setImportError(
            "Không thể đọc file XLSX. Vui lòng kiểm tra định dạng.",
          );
        }
      };
      ...
      reader.onload = (event) => {
        try {
          ...
          if (importMode === "json") {
            ...
            if (lines.length < 2) {
              setImportError(
                "File CSV cần có header và ít nhất 1 dòng dữ liệu",
              );
              return;
            }
            ...
            const items: ImportVocabularyItem[] = lines.slice(1).map((line) => {
              ...
              headers.forEach((header, index) => {
                if (header === "hanzi") item.hanzi = values[index];
                  ...
                  ["meaningvi", "meaning_vi", "vietnamese", "nghĩa"].includes(
                    header,
                  )
                )
                  item.meaningVi = values[index];
                else if (
                  ["meaningen", "meaning_en", "english"].includes(header)
                )
                  item.meaningEn = values[index];
                else if (
                  ["partofspeech", "part_of_speech", "pos"].includes(header)
                )
                  item.partOfSpeech = values[index];
                else if (["hsklevel", "hsk_level", "hsk"].includes(header))
                  item.hskLevel = parseInt(values[index]) || 1;
  ...
  const handleImportConfirm = async () => {
    ...
    setImportProgress({
      active: true,
      message: `Đang kiểm tra từ vựng trùng lặp...`,
    });
    try {
  ...
  const executeImport = async (duplicateAction: "skip" | "overwrite") => {
    ...
    setImportProgress({
      active: true,
      message: `Bắt đầu import ${importData.length} từ vựng...`,
      percent: 0,
    });
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (result.errorDetails)
          aggregatedResult.errorDetails.push(...result.errorDetails);
        if (result.skippedDetails)
          aggregatedResult.skippedDetails.push(...result.skippedDetails);
  ...
  const handleUpdateConfirm = async () => {
    ...
    setImportProgress({
      active: true,
      message: `Đang cập nhật ${updateData.length} từ vựng...`,
    });
    try {
    ...
    const hsk1Data = [
      {
        ...
        NGHĨA: "quý, yêu, thương",
        "VÍ DỤ (CHỮ HÁN)": "我爱你。",
        "PHIÊN ÂM": "Wǒ ài nǐ.",
        DỊCH: "Tôi yêu bạn.",
        "TỪ CẬN NGHĨA": "爱情, 喜爱, 热爱",
        "TỪ TRÁI NGHĨA": "恨, 讨厌",
      },
      {
        STT: 2,
  ...
  const highlightText = (text: string, query: string) => {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <span className="bg-amber-500/40 text-white rounded px-0.5">
  ...
  const columns = [
    {
      ...
      render: (vocab: Vocabulary) => (
        <div className="flex flex-col min-w-0">
          <span
            className="text-base sm:text-xl font-bold text-text-base font-chinese whitespace-nowrap overflow-hidden text-ellipsis"
            lang="zh-CN"
          >
            {searchQuery
              ? highlightText(vocab.hanzi, searchQuery)
              : vocab.hanzi}
          </span>
          <span className="text-xs text-primary/80 mt-0.5 font-pinyin tracking-tight whitespace-nowrap overflow-hidden text-ellipsis">
            {searchQuery
              ? highlightText(vocab.pinyin, searchQuery)
              : vocab.pinyin}
          </span>
        </div>
      ),
        ...
        return (
          <div className="flex flex-wrap gap-1 max-w-[90px]">
            {tags.map((tag, idx) => (
              <span
                key={idx}
                className="text-[9px] font-bold px-1.5 py-0.5 bg-primary/10 text-primary rounded-full border border-primary/20 uppercase tracking-tighter"
              >
                {tag}
              </span>
            ))}
    ...
    },
    {
      ...
      width: "280px",
      render: (vocab: Vocabulary) => {
        const multi = (vocab.meanings as MeaningVariant[] | undefined) || [];
        if (multi.length > 0) {
          return (
            <div className="flex flex-col gap-1 min-w-[200px]">
              {/* Primary meaning */}
              {renderFormattedMeaning(
                vocab.meaningVi || vocab.meaningEn || "-",
              )}
              ...
              {multi.map((m, idx) => (
                <div
                  key={idx}
                  className="border-t border-border-color/30 pt-1 mt-1"
                >
                  <div className="text-[10px] text-primary/60 font-pinyin mb-0.5">
                    ({m.pinyin})
                  </div>
                  {renderFormattedMeaning(
                    `${m.partOfSpeech ? m.partOfSpeech + ": " : ""}${Array.isArray(m.meanings) ? m.meanings.join(", ") : m.meanings}`,
                  )}
                </div>
              ))}
            </div>
    ...
    },
    {
      ...
      hideOnMobile: true,
      render: (vocab: Vocabulary) => {
        const examples = vocab.examples as Array<{
          chinese?: string;
          pinyin?: string;
          vietnamese?: string;
        }> | null;
        ...
        return (
          <div className="text-sm">
            <p className="text-text-base font-chinese">
              {highlightWord(first.chinese || "", vocab.hanzi)}
            </p>
            <p className="text-primary/70 text-xs font-pinyin">
              {highlightWord(first.pinyin || "", vocab.pinyin)}
            </p>
    ...
    },
    {
      ...
      hideOnMobile: true,
      render: (vocab: Vocabulary) => {
        const synonyms = vocab.synonyms as Array<{
          hanzi?: string;
          meaning?: string;
        }> | null;
        const antonyms = vocab.antonyms as Array<{
          hanzi?: string;
          meaning?: string;
        }> | null;
          ...
          <div className="text-xs space-y-1">
            {hasSynonyms && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-green-300 font-chinese">
                  {synonyms
                    .slice(0, 3)
                    .map((s) => s.hanzi)
                    .join(", ")}
            ...
            {hasAntonyms && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-red-300 font-chinese">
                  {antonyms
                    .slice(0, 3)
                    .map((a) => a.hanzi)
                    .join(", ")}
  ...
  const actions = (vocab: Vocabulary) => (
    ...
    >
      <SpeakerButton text={vocab.hanzi} size="sm" />
      <button
        onClick={(e) => {
          e.stopPropagation();
      ...
      actions={
        <div className="flex flex-wrap items-center justify-end gap-2">
          <div className="relative group">
            <button className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-surface-dark border border-border-color text-text-base text-xs sm:text-sm font-medium rounded-lg hover:bg-surface-highlight transition-colors">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-4 lg:grid-cols-8 gap-2 mb-4">
        {[1, 2, 3, 4, 5, 6, 7, 0].map((level) => {
          ...
          return (
            ...
            >
              <p
                className={`text-sm sm:text-xl font-bold ${filterHsk === level ? "text-current" : "text-text-base"}`}
              >
                {count}
              </p>
              <p className="text-[8px] sm:text-[10px] text-text-secondary leading-tight line-clamp-1 font-medium">
                {label}
              </p>
            </button>
          );
      ...
      <div className="flex items-center justify-between mb-3 sm:mb-6">
        <p className="text-xs sm:text-sm text-text-secondary">
          ...
          <span className="text-text-base font-bold">
            {Object.values(hskStats).reduce((a, b) => a + b, 0)}
          </span>{" "}
          từ vựng
        </p>
        <button
        ...
        {selectedVocab && (
          <div className="space-y-6">
            ...
            <div className="bg-surface-highlight/10 border border-border-color/30 rounded-2xl sm:rounded-3xl p-4 sm:p-8 text-center relative overflow-hidden backdrop-blur-sm">
              ...
              <h2 className="text-4xl sm:text-7xl font-chinese text-text-base mb-2 leading-none drop-shadow-lg">
                {selectedVocab.hanzi}
              </h2>
              <p className="text-xl sm:text-3xl font-pinyin text-primary mb-4 sm:mb-6 tracking-wide drop-shadow-md">
                {selectedVocab.pinyin}
              </p>

              <div className="flex flex-wrap justify-center gap-3">
                {selectedVocab.hskLevel > 0 && (
                  <div
                    className={`px-4 py-1.5 ${HSK_COLORS[selectedVocab.hskLevel] || "bg-gray-500"} bg-opacity-20 border-none text-text-base text-xs font-black rounded-full uppercase tracking-widest shadow-sm flex items-center gap-2`}
                  >
              ...
              {(selectedVocab.radical || selectedVocab.radicalMeaning) && (
                <div className="bg-surface-dark border border-border-color rounded-xl p-4">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-xl sm:text-2xl font-chinese text-text-base">
                      {selectedVocab.radical || "-"}
                    </span>
                    <span className="text-xs sm:text-sm text-text-secondary">
                      {selectedVocab.radicalMeaning}
                    </span>
                  </div>
                </div>
              )}
            ...
            {selectedVocab.mnemonic && (
              <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-4">
                ...
                <p className="text-sm text-text-base italic whitespace-pre-wrap">
                  {selectedVocab.mnemonic}
                </p>
              </div>
            )}
                ...
                <div className="grid gap-4">
                  {(() => {
                    ...
                    selectedVocab.examples.forEach((ex) => {
                      ...
                      const hSent = (ex.chinese || "")
                        .split(/(?<=[。！？])\s*/)
                        .filter((s) => s.trim());
                      // Split by Western full stops/exclamation/question + space for Pinyin/Vietnamese
                      const pSent = (ex.pinyin || "")
                        .split(/(?<=[.!?])\s+/)
                        .filter((s) => s.trim());
                      const vSent = (ex.vietnamese || "")
                        .split(/(?<=[.!?])\s+/)
                        .filter((s) => s.trim());

                      if (
                        hSent.length > 1 &&
                        hSent.length === pSent.length &&
                        hSent.length === vSent.length
                      ) {
                        hSent.forEach((h, i) => {
                          splitItems.push({
                            chinese: h,
                    ...
                    return splitItems.map((ex, idx) => (
                      <div
                        key={idx}
                        className="bg-surface-highlight/10 border border-border-color/40 rounded-3xl p-6 hover:border-primary/30 transition-all group relative overflow-hidden"
                      >
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="shrink-0 group-hover:scale-110 transition-transform">
                            <SpeakerButton
                              text={ex.chinese || ""}
                              size="sm"
                              className="size-8 bg-primary/10 hover:bg-primary/20 text-primary rounded-lg transition-all"
                            />
                          </div>
                        </div>
                        <div className="space-y-4">
                          <p
                            className="text-lg sm:text-2xl font-chinese text-text-base leading-snug"
                            lang="zh-CN"
                          >
                            {highlightWord(
                              ex.chinese || "",
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedVocab.synonyms.map((s, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-1 bg-green-500/10 text-green-400 rounded-full border border-green-500/20 text-sm font-chinese"
                      >
                        {s.hanzi}
                      </span>
                    ))}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedVocab.antonyms.map((a, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-1 bg-red-500/10 text-red-400 rounded-full border border-red-500/20 text-sm font-chinese"
                      >
                        {a.hanzi}
                      </span>
                    ))}
            ...
            {selectedVocab.tags && selectedVocab.tags.length > 0 && (
              <div className="pt-4 border-t border-border-color flex flex-wrap gap-2">
                {selectedVocab.tags.map((tag, idx) => (
                  <span
                    key={idx}
                    className="text-[10px] text-text-secondary bg-surface-dark px-2 py-0.5 rounded border border-border-color"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="flex items-center justify-between text-xs font-medium text-text-secondary mb-1">
                  ...
                  >
                    {isLookingUp ? (
                      <span className="size-3 border-2 border-amber-400 border-t-transparent rounded-full animate-spin" />
                    ) : (
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 space-y-4">
            ...
            <div className="space-y-4">
              {meaningEntries.map((entry, idx) => (
                <div
                  key={idx}
                  className="relative bg-background-dark/50 p-3 rounded-lg border border-border-color/50 space-y-3"
                >
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      <label className="block text-[10px] font-medium text-text-secondary uppercase mb-1.5">
                        Loại từ (Chọn nhiều)
                      </label>
                      <div className="flex flex-wrap gap-1.5 p-1 bg-background-dark/30 rounded border border-border-color/30">
                        {POS_OPTIONS.map((pos) => {
                          const currentTags = entry.partOfSpeech
                            .split(/[\/,]/)
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                const currentTagsRaw = entry.partOfSpeech
                                  .split(/[\/,]/)
                                  .map((t) => t.trim())
                                  .filter(Boolean);
                                let nextTags;
                                if (isSelected) {
                                  nextTags = currentTagsRaw.filter(
                                    (p) =>
                                      p.toLowerCase() !== pos.toLowerCase(),
                                  );
                                } else {
                                  nextTags = [...currentTagsRaw, pos];
                                }
                                const newEntries = [...meaningEntries];
                                newEntries[idx].partOfSpeech =
                                  nextTags.join(", ");
                    ...
                    <div className="col-span-2">
                      <label className="block text-[10px] font-medium text-text-secondary uppercase mb-1">
                        Pinyin (nếu khác {formData.pinyin})
                      </label>
                      <input
                        type="text"
                        value={entry.pinyin}
                  ...
                  <div>
                    <label className="block text-[10px] font-medium text-text-secondary uppercase mb-1">
                      Dịch nghĩa (cách nhau bởi dấu chấm phẩy)
                    </label>
                    <textarea
                ...
                >
                  {exampleEntries.length > 1 && (
                    <button
                      ...
                      onClick={() =>
                        setExampleEntries(
                          exampleEntries.filter((_, i) => i !== idx),
                        )
                      }
              ...
              <div className="max-h-48 overflow-y-auto border border-border-color rounded-lg">
                <table className="w-full text-sm">
                  <thead className="bg-background-dark sticky top-0">
                    <tr>
                      <th className="px-3 py-2 text-left text-text-secondary">
                        Từ vựng
                      </th>
                      <th className="px-3 py-2 text-left text-text-secondary">
                        Pinyin
                  ...
                  <tbody className="divide-y divide-border-color">
                    {importData.slice(0, 10).map((item, index) => (
                      ...
                      >
                        <td className="px-3 py-2 text-text-base font-chinese">
                          {item.hanzi}
                        </td>
                        <td className="px-3 py-2 text-primary font-pinyin">
                          {item.pinyin}
                        </td>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
              ...
              <p className="text-sm text-text-secondary">
                ...
                <strong className="text-white">
                  {duplicateConfirmImport.total}
                </strong>{" "}
                từ vựng. Phát hiện{" "}
                <strong className="text-amber-400">
                  {duplicateConfirmImport.duplicates.length}
                </strong>{" "}
                Hán tự đã tồn tại trong hệ thống.
              </p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-2 bg-background-dark border border-border-color rounded-lg">
                {duplicateConfirmImport.duplicates
                  .slice(0, 20)
                  .map((hanzi, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-1 bg-surface-highlight text-primary rounded text-sm font-chinese"
                    >
                      {hanzi}
                    </span>
                  ))}
            ...
            <div className="space-y-3">
              ...
              {importResult.skipped > 0 && (
                <div className="flex items-center gap-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-text-secondary">
                      Bỏ qua (đã tồn tại / không thao tác)
                    </p>
              ...
              importResult.errorDetails.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-red-400 flex items-center gap-1">
                    <Icon name="error" size="sm" /> Chi tiết lỗi (
                  ...
                  <div className="max-h-40 overflow-y-auto rounded-lg border border-red-500/20 bg-red-500/5">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-surface-secondary">
                        <tr>
                          <th className="text-left p-2 text-text-secondary">
                            Hanzi
                          </th>
                          <th className="text-left p-2 text-text-secondary">
                            Lỗi
                      ...
                      <tbody>
                        {importResult.errorDetails.map((e, i) => (
                          <tr key={i} className="border-t border-red-500/10">
                            <td className="p-2 font-chinese text-text-base">
                              {e.hanzi}
                            </td>
              ...
              importResult.skippedDetails.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-amber-400 flex items-center gap-1">
                    <Icon name="info" size="sm" /> Chi tiết bỏ qua (
                    {importResult.skippedDetails.length})
                  </h4>
                      ...
                      <tbody>
                        ...
                        {importResult.skippedDetails.length > 100 && (
                          <tr>
                            <td
                              colSpan={2}
                              className="p-2 text-center text-text-secondary italic"
                            >
                              ... và {importResult.skippedDetails.length - 100}{" "}
                              từ khác
                            </td>
                          </tr>
                        )}
          ...
          <div className="space-y-4">
            ...
            <div className="text-center space-y-2">
              <p className="text-text-base">
                Từ vựng{" "}
                <span className="text-2xl font-bold text-amber-500 font-chinese">
                  {duplicateConfirm.vocab.hanzi}
                </span>{" "}
                đã tồn tại trong hệ thống.
              </p>
              <p className="text-sm text-text-secondary">
                Bạn có muốn chuyển sang chế độ{" "}
                <strong className="text-amber-400">Sửa</strong> để cập nhật từ
                này không?
              </p>
            </div>
        ...
        footer={
          <>
            ...
            <button
              ...
              disabled={
                isSaving ||
                deleteAllConfirmText.toLowerCase() !== "xác nhận xóa"
              }
      ...
      >
        <div className="space-y-4">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-text-secondary">
              Hành động này không thể hoàn tác. Bạn sẽ cần import lại file XLSX
              sau khi xóa.
            </p>
          </div>
          <div className="mt-4">
            <label className="block text-sm text-text-secondary mb-2 text-center">
              Vui lòng nhập{" "}
              <strong className="text-red-500">xác nhận xóa</strong> để tiếp tục
            </label>
            <input
              type="text"
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                ...
                <ul className="text-text-secondary space-y-1 list-disc list-inside">
                  <li>
                    Upload file XLSX chứa các từ cần{" "}
                    <strong className="text-purple-400">cập nhật</strong>
                  </li>
                  <li>
                    Chỉ những từ{" "}
                    <strong className="text-purple-400">đã tồn tại</strong> sẽ
                    được cập nhật
                  </li>
                  <li>Từ không có trong cơ sở dữ liệu sẽ bị bỏ qua</li>
                  <li>
                    Cột <code className="text-purple-400">hanzi</code> là key để
                    tìm từ cần update
                  </li>
                </ul>
              </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border-color">
                    {updateData.slice(0, 10).map((item, index) => (
                      <tr
                        key={index}
                        className="border-b border-border-color/30 last:border-0 hover:bg-surface-highlight/5 transition-colors"
                      >
                        <td className="px-3 py-2 text-text-base font-chinese">
                          {item.hanzi}
                        </td>
                        ...
                        <td className="px-3 py-2 text-text-base font-bold">
                          {item.hskLevel}
                        </td>
                      </tr>
                    ))}
            ...
            {updateResult.updated === 0 && updateResult.skipped > 0 && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                <p className="text-sm text-amber-400 flex items-center gap-2">
                  <Icon name="lightbulb" size="sm" />
                  Không có từ nào được cập nhật vì tất cả từ trong file chưa tồn
                  tại trong database.
                </p>
              </div>
            )}
