...
export default function AdminEmailsPage() {
  ...
  const handleSave = async () => {
    ...
    const saveTemplate = async (
      htmlBodyToSave: string,
      designJsonToSave: any,
    ) => {
      setSaving(true);
      try {
        ...
        setSelectedTemplate(null);
        showToast(
          selectedTemplate ? "Đã cập nhật template" : "Đã tạo template mới",
          "success",
        );
      } catch (error) {
            ...
            <div className="bg-surface-dark rounded-xl border border-border-color p-3 sm:p-4 shadow-sm">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="size-8 sm:size-10 bg-primary/10 rounded-lg flex items-center justify-center shrink-0">
                  <Icon
                    name="send"
                    className="text-sm sm:text-xl text-primary"
                  />
                </div>
                <div className="min-w-0">
                  <p className="text-lg sm:text-2xl font-bold text-text-base truncate">
                    {statistics.summary.totalSent}
                  </p>
            ...
            <div className="bg-surface-dark rounded-xl border border-border-color p-3 sm:p-4 shadow-sm">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="size-8 sm:size-10 bg-green-500/10 rounded-lg flex items-center justify-center shrink-0">
                  <Icon
                    name="visibility"
                    className="text-sm sm:text-xl text-green-400"
                  />
                </div>
                <div className="min-w-0">
                  <p className="text-lg sm:text-2xl font-bold text-text-base truncate">
                    {statistics.summary.openRate}%
                  </p>
                  <p className="text-[10px] sm:text-xs text-text-secondary truncate">
                    Tỷ lệ mở
                  </p>
            ...
            <div className="bg-surface-dark rounded-xl border border-border-color p-3 sm:p-4 shadow-sm">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="size-8 sm:size-10 bg-amber-500/10 rounded-lg flex items-center justify-center shrink-0">
                  <Icon
                    name="touch_app"
                    className="text-sm sm:text-xl text-amber-400"
                  />
                </div>
                <div className="min-w-0">
                  <p className="text-lg sm:text-2xl font-bold text-text-base truncate">
                    {statistics.summary.clickRate}%
                  </p>
                  <p className="text-[10px] sm:text-xs text-text-secondary truncate">
                    Tỷ lệ click
                  </p>
            ...
            <div className="bg-surface-dark rounded-xl border border-border-color p-3 sm:p-4 shadow-sm">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="size-8 sm:size-10 bg-red-500/10 rounded-lg flex items-center justify-center shrink-0">
                  <Icon
                    name="error"
                    className="text-sm sm:text-xl text-red-400"
                  />
                </div>
                <div className="min-w-0">
                  <p className="text-lg sm:text-2xl font-bold text-text-base truncate">
                    {statistics.summary.totalFailed}
                  </p>
                  <p className="text-[10px] sm:text-xs text-text-secondary truncate">
                    Thất bại
                  </p>
            ...
            <div className="lg:col-span-2 bg-surface-dark rounded-xl border border-border-color p-6 shadow-sm">
              ...
              <div className="bg-background-dark rounded-xl border border-border-color p-4 mb-6">
                <h3 className="text-sm font-bold text-text-base mb-4 flex items-center gap-2">
                  <Icon name="schedule" className="text-primary" /> Điều kiện
                  gửi
                </h3>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            triggerDays: parseInt(e.target.value) || 3,
                          })
                        }
                        className="w-full px-3 py-2 bg-surface-dark border border-border-color rounded-lg text-text-base text-sm"
                      />
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            triggerDayOfWeek: parseInt(e.target.value),
                          })
                        }
                        className="w-full px-3 py-2 bg-surface-dark border border-border-color rounded-lg text-text-base text-sm"
                      >
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="group relative flex items-center">
                    <Icon
                      name="help_outline"
                      className="text-primary text-xl cursor-help"
                    />
                    <div className="absolute top-full right-0 mt-2 w-72 bg-surface-dark border border-primary/20 p-3 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 tooltip-triangle">
                      <p className="text-xs text-text-secondary">
                        ...
                        Kéo các khối tử bên phải (Text, Hình ảnh, Nút...) vào
                        vùng thiết kế ở giữa. Bạn có thể sử dụng các biến động
                        (Ví dụ:{" "}
                        <code className="text-primary bg-primary/10 px-1 rounded">
                          {"{{userName}}"}
                        </code>
                        ) trực tiếp trong các khối văn bản.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {saving ? (
                    <>
                      <Icon name="sync" className="animate-spin mr-2" /> Đang
                      lưu...
                    </>
                  ) : (
                    <>
          ...
          <div className="space-y-4">
            {templates.length === 0 ? (
              <div className="bg-surface-dark rounded-xl border border-border-color p-12 text-center">
                <Icon
                  name="mail"
                  className="text-5xl text-text-secondary/30 mb-4"
                />
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-start sm:items-center gap-3 sm:gap-4 flex-1 min-w-0">
                      <div
                        className={`p-2 sm:p-3 rounded-xl flex items-center justify-center shrink-0 ${template.isActive ? "bg-amber-500/10" : "bg-gray-500/10"}`}
                      >
                        <Icon
                          name={
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="font-bold text-text-base text-sm sm:text-base truncate">
                            {template.nameVi}
                          </h3>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 shrink-0 ml-2">
                      ...
                      >
                        <Icon
                          name={testSending === template.code ? "sync" : "send"}
                          className={`text-sm sm:text-base text-primary ${testSending === template.code ? "animate-spin" : ""}`}
                        />
                      </button>
                      <button
                      ...
                      >
                        <Icon
                          name="edit"
                          className="text-sm sm:text-base text-amber-400"
                        />
                      </button>
                      <button
                      ...
                      >
                        <Icon
                          name="delete"
                          className="text-sm sm:text-base text-red-400"
                        />
                      </button>
                    </div>
                  </div>
