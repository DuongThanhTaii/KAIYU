...
import { getOverviewStats, getActivityStats, type OverviewStats, type ActivityData } from '@/services/adminApi';
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-surface-dark rounded-xl border border-border-color p-4 sm:p-6 shadow-sm transition-colors overflow-hidden">
                    ...
                    {chartLoading ? (
                        ...
                        <div className="h-52">
                            {chartData.length > 0 ? (() => {
                                const maxValue = Math.max(...chartData.map(d => Math.max(d.newUsers, d.videoViews)), 1);
                                ...
                                return (
                                    <div className="flex h-full">
                                        {/* Y-axis */}
                                        <div className="flex flex-col justify-between items-end pr-2 text-[10px] text-text-secondary" style={{ height: `${chartHeight}px` }}>
                                            <span>{maxValue}</span>
                                            ...
                                            <div className="relative border-l border-b border-border-color overflow-hidden" style={{ height: `${chartHeight}px` }}>
                                                ...
                                                <div className="absolute inset-0 flex items-end">
                                                    {chartData.map((day, index) => {
                                                        const userHeightPx = (day.newUsers / maxValue) * chartHeight;
                                                        const videoHeightPx = (day.videoViews / maxValue) * chartHeight;
                                                        const isToday = day.date === today;
                                                        return (
                                                            <div key={index} className="flex-1 flex items-end justify-center">
                                                                <div className={`flex items-end gap-1 group ${isToday ? 'relative z-10' : ''}`}>
                                                                    {/* Today indicator */}
                                                                    {isToday && (
                                                                    ...
                                                                    <div className="relative flex flex-col items-center">
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            style={{ height: `${videoHeightPx}px`, minHeight: day.videoViews > 0 ? '4px' : '0' }}
                                                                        />
                                                                    </div>
                                                                </div>
                                                            </div>
                                                ...
                                                {chartData.map((day, index) => {
                                                    ...
                                                    return (
                                                        <div key={index} className="flex-1 flex justify-center">
                                                            <span className={isToday ? 'text-amber-400 font-bold' : ''}>
                                            ...
                                            <tbody>
                                                {(() => {
                                                    ...
                                                    return weeks.map((week, weekIndex) => (
                                                        <tr key={weekIndex} className="border-b border-border-color/20">
                                                                ...
                                                                <td key={dayIndex} className="p-1.5 text-center">
                                                                    <div className="flex flex-col items-center gap-0.5">
                                                                        ...
                                                                        <div className="flex gap-0.5 justify-center">
                                                                            {day.newUsers > 0 && (
                                                                                <span className="px-1 py-0.5 bg-amber-500/20 text-amber-400 rounded text-[9px]" title="Người dùng mới">
                                                                                    {day.newUsers}
                                                                                </span>
                                                                            )}
                                                                            {day.videoViews > 0 && (
                                                                                <span className="px-1 py-0.5 bg-primary/20 text-primary rounded text-[9px]" title="Lượt xem">
                                                                                    {day.videoViews}
                                                                                </span>
                                                                            )}
                                                                        </div>
                                                            ...
                                                            {Array.from({ length: 7 - week.length }).map((_, i) => (
                                                                <td key={`empty-${i}`} className="p-1.5 text-center text-text-secondary/20">
                                                                    -
                                                                </td>
                                                            ))}
                                                        </tr>
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 mt-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-text-secondary">Lượt xem video</span>
                        </div>
                    </div>
                </div>
