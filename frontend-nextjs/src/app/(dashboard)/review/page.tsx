...
import {
  flashcardApi,
  type FlashcardReview as FlashcardType,
  type FlashcardStats,
  type SRSRating,
...
import {
  dictionaryApi,
  type ExampleSentence as DictExample,
} from "@/services/dictionaryApi";

interface ReviewExampleItem {
  chinese?: string;
  hanzi?: string;
  pinyin?: string;
  vietnamese?: string;
...
interface ReviewVocabWithWord {
  hanzi?: string;
  word?: string;
  pinyin?: string;
  examples?: ReviewExampleItem[];
  meaningVi?: string;
  meaningEn?: string;
}

type SubtitleWithTokens = Subtitle & { tokens?: string[] };
...
export default function FlashcardReviewPage() {
  ...
  const [fallbackExamples, setFallbackExamples] = useState<
    Record<string, DictExample[]>
  >({});

  // === View Mode ===
  const [viewMode, setViewMode] = useState<
    "srs" | "mastered" | "review" | "matching"
  >("srs");
  ...
  const [masteredSlideshow, setMasteredSlideshow] = useState<
    FlashcardType[] | null
  >(null);
  ...
  const getVocab = (card: FlashcardType | undefined) =>
    card?.vocabulary || card?.word;
  ...
  const getActiveCard = (): FlashcardType | undefined => {
    if (viewMode === "srs")
      return historyIndex < sessionHistory.length
        ? sessionHistory[historyIndex]
        : currentCard;
    if (viewMode === "mastered" && masteredSlideshow)
      return masteredSlideshow[masteredSlideshowIndex];
    if (viewMode === "review")
      return reviewHistoryIndex < reviewHistory.length
        ? reviewHistory[reviewHistoryIndex]
        : reviewPool[reviewPoolIndex];
    return undefined;
  };
  const activeCard = getActiveCard();

  // === Displayed cards per mode ===
  const srsDisplayedCard =
    historyIndex < sessionHistory.length
      ? sessionHistory[historyIndex]
      : currentCard;
  const reviewDisplayedCard =
    reviewHistoryIndex < reviewHistory.length
      ? reviewHistory[reviewHistoryIndex]
      : reviewPool[reviewPoolIndex];
  const masteredDisplayedCard = masteredSlideshow
    ? masteredSlideshow[masteredSlideshowIndex]
    : undefined;
  ...
  useEffect(() => {
    const fetchSubtitlesIfNeeded = async () => {
      if (
        !activeCard ||
        activeCard.sourceMeaning ||
        !activeCard.sourceVideoUrl ||
        activeCard.sourceTimestamp === undefined
      )
        return;
      ...
      if (!videoId && activeCard.sourceVideoUrl) {
        try {
          const searchResult = await videoApi.getAll({
            search: activeCard.sourceVideoUrl,
            limit: 1,
          });
          if (searchResult.data.length > 0) videoId = searchResult.data[0].id;
        } catch (e) {
          console.error("Failed to resolve video ID by URL:", e);
        }
      }
      if (
        !videoId ||
        videoSubtitles[videoId] ||
        !/^[0-9a-fA-F]{24}$/.test(videoId)
      )
        return;
      try {
        ...
        console.error("Failed to fetch fallback subtitles:", error);
      }
    };
    const fetchExamplesIfNeeded = async () => {
      ...
      try {
        ...
        console.error("Failed to fetch fallback examples:", error);
      }
    };
  ...
  const handleSRSRating = async (rating: SRSRating) => {
    ...
    try {
      ...
      if (rating === "again") {
        ...
        setWrongWords((prev) => [
          ...prev.filter((w) => w.id !== currentCard.id),
          currentCard,
        ]);
  ...
  const navigateHistory = (direction: "prev" | "next") => {
    if (direction === "prev") {
      ...
      if (historyIndex < sessionHistory.length - 1) {
        ...
        setHistoryIndex(sessionHistory.length);
        setIsRevealed(false);
      }
    }
  };
  ...
  const navigateReviewHistory = (direction: "prev" | "next") => {
    if (direction === "prev") {
      if (reviewHistoryIndex > 0) {
        setReviewHistoryIndex((prev) => prev - 1);
        setReviewRevealed(false);
      }
    } else {
      if (reviewHistoryIndex < reviewHistory.length - 1) {
        ...
        setReviewHistoryIndex(reviewHistory.length);
        setReviewRevealed(false);
      }
    }
  };
  ...
  const navigateMasteredSlideshow = (direction: "prev" | "next") => {
    ...
    if (direction === "prev") {
      if (masteredSlideshowIndex > 0) {
        setMasteredSlideshowIndex((prev) => prev - 1);
        setMasteredRevealed(false);
      }
    } else {
      if (masteredSlideshowIndex < masteredSlideshow.length - 1) {
        setMasteredSlideshowIndex((prev) => prev + 1);
        setMasteredRevealed(false);
      }
    }
  ...
  const renderCardFaces = (
    ...
    return (
      ...
      >
        ...
        >
          <h1
            className="text-8xl md:text-9xl font-bold text-text-base tracking-tight font-chinese mb-6 text-glow"
            lang="zh-CN"
          >
            {vocab.hanzi}
          </h1>
          <div className="level-badge px-3 py-1 rounded-full flex items-center gap-3 mb-6">
            <span className="text-lg font-bold text-primary">
              HSK {vocab.hskLevel || card?.level || 1}
            </span>
          </div>
        ...
        >
          <div className="flex-1 flex flex-col items-center p-6 md:p-8 pt-6 overflow-y-auto">
            ...
            <div className="text-center mb-4">
              <h2
                className="text-5xl md:text-6xl font-bold text-text-base font-chinese mb-2"
                lang="zh-CN"
              >
                {vocab.hanzi}
              </h2>
              <div className="flex flex-col items-center justify-center gap-1">
                <span className="text-2xl md:text-3xl text-primary font-semibold font-pinyin tracking-tight">
                  {vocab.pinyin || card?.sourcePinyin}
                </span>
                ...
                vocab.meaningVi.includes("1.") ? (
                  vocab.meaningVi
                    ...
                    .map((part: string, i: number) => (
                      <div
                        key={i}
                        className="text-xl font-bold text-text-base leading-relaxed"
                      >
                        {renderFormattedMeaning(part)}
                      </div>
                    ))
                ) : (
                  <p className="text-xl md:text-2xl text-text-base font-bold">
                    {vocab.meaningVi}
                  </p>
                )
              ) : (
            ...
            {/* Example */}
            {(() => {
              const reviewVocab = vocab as ReviewVocabWithWord;
              const hanziForLookup =
                reviewVocab.hanzi || reviewVocab.word || "";
              const examples =
                vocab.examples && vocab.examples.length > 0
                  ? vocab.examples
                  : fallbackExamples[hanziForLookup] || [];
              if (examples.length === 0) return null;
              const ex = examples[0] as ReviewExampleItem;
              const exHanzi = ex.chinese || ex.hanzi;
              const exPinyin = ex.pinyin;
              const exMeaning =
                (ex.vietnamese && ex.vietnamese.trim()) ||
                (ex.translation && ex.translation.trim()) ||
                (ex.meaningVi && ex.meaningVi.trim()) ||
                ex.meaning;
            ...
            {(() => {
              ...
              return (
                ...
                >
                  ...
                  {card?.sourceVideoUrl &&
                    card?.sourceTimestamp !== undefined && (
                      <div
                        onClick={(e) => e.stopPropagation()}
                        className="rounded-xl overflow-hidden shadow-md"
                      >
                        <VideoClipPlayer
                          videoUrl={card.sourceVideoUrl}
                          startTime={card.sourceTimestamp}
                          duration={5}
                        />
                      </div>
                    )}
                  ...
                  <div className="space-y-2">
                    {card?.sourceSentence && (
                      <p
                        className="text-text-base font-chinese text-lg leading-relaxed"
                        lang="zh-CN"
                      >
                    ...
                    {(() => {
                      const vId = card?.sourceVideoUrl
                        ? videoApi.getYouTubeId(card.sourceVideoUrl)
                        : null;
                      const subs = vId ? videoSubtitles[vId] : null;
                      const match = subs?.find(
                        (s: Subtitle) =>
                          Math.abs(s.startTime - (card?.sourceTimestamp || 0)) <
                          3.0,
                      );
                      const tokens =
                        card?.sourceTokens ||
                        (match as SubtitleWithTokens | undefined)?.tokens;
                      const meaningText =
                        card?.sourceMeaning || match?.meaningVi;
                      return (
                        <>
                          {card?.sourcePinyin && (
              ...
              revealed ? (
                <div className="flex items-center gap-2 animate-pulse text-primary font-bold">
                  ...
                  <span className="text-sm">
                    {isHistory
                      ? "Đã xem đáp án"
                      : "Vuốt để trả lời • Nhấn để lật lại"}
                  </span>
                </div>
              ) : (
  ...
  const getHskCards = (hsk: number) =>
    masteredCards.filter((c) => getVocab(c)?.hskLevel === hsk);
  const otherCards = masteredCards.filter((c) => {
    const lvl = getVocab(c)?.hskLevel;
    return !lvl || lvl < 1 || lvl > 6;
  });

  // === Review pool size for sidebar ===
  const reviewPoolSize =
    correctWords.length + wrongWords.length + Math.min(5, masteredCards.length);
      ...
      <header className="sticky top-0 z-50 bg-[var(--color-background-dark)]/80 backdrop-blur-md border-b border-border-color px-4 md:px-8 py-3">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              href="/dashboard"
              className="size-10 flex items-center justify-center rounded-full hover:bg-surface-highlight transition-colors text-text-base"
            >
            ...
            <div className="flex flex-col">
              ...
              <h1 className="text-text-base text-lg font-bold leading-tight">
                Flashcards
              </h1>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full flex-1">
          ...
          <aside className="lg:col-span-3 xl:col-span-3 flex flex-col gap-4">
            <div className="bg-surface-dark rounded-[2rem] p-6 border border-border-color flex flex-col gap-5 shadow-lg relative overflow-hidden">
              ...
              <div className="flex flex-col gap-3 relative z-10">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`size-8 rounded-full flex items-center justify-center ${viewMode === "srs" && !showMatchingGame ? "bg-primary text-on-primary" : "bg-surface-highlight text-text-secondary"}`}
                    >
                  ...
                  <span className="text-text-base text-xl font-bold">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`size-8 rounded-full flex items-center justify-center ${viewMode === "mastered" ? "bg-emerald-500 text-white" : "bg-surface-highlight text-text-secondary"}`}
                    >
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`size-8 rounded-full flex items-center justify-center ${viewMode === "review" ? "bg-orange-500 text-white" : "bg-surface-highlight text-text-secondary"}`}
                    >
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`size-8 rounded-full flex items-center justify-center ${viewMode === "matching" ? "bg-purple-500 text-white" : "bg-surface-highlight text-text-secondary"}`}
                    >
            ...
            <div className="bg-surface-dark/50 rounded-2xl p-4 border border-border-color flex justify-between items-center">
              <span className="text-text-secondary text-xs uppercase font-bold tracking-widest">
                Hôm nay
              </span>
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  <Icon name="cancel" size="sm" className="text-rose-400" />
                  <span className="text-text-base text-sm font-bold">
                    {sessionWrongCount}
                  </span>
                </div>
          ...
          <section className="lg:col-span-9 xl:col-span-9 flex flex-col items-center justify-start min-h-[600px] relative pb-4">
            ...
            {showMatchingGame ? (
              <MatchingGame
                wrongWords={
                  wrongWords.length > 0 ? wrongWords : masteredCards.slice(0, 5)
                }
                correctWords={
                  correctWords.length > 0
                    ? correctWords
                    : masteredCards.slice(5, 10)
                }
                onComplete={() => {
                  setShowMatchingGame(false);
                  if (viewMode === "review") {
              ...
              masteredSlideshow ? (
                ...
                <div className="w-full max-w-3xl animate-fade-in flex flex-col items-center relative">
                  <div className="w-full flex items-center justify-between mb-4">
                    <button
                      onClick={() => setMasteredSlideshow(null)}
                    ...
                    <span className="text-text-secondary text-sm font-bold">
                      {masteredSlideshowIndex + 1} / {masteredSlideshow.length}
                    </span>
                  </div>
                  {/* Navigation Arrows */}
                  <div className="absolute top-1/2 left-0 right-0 -translate-y-1/2 flex justify-between w-full px-4 pointer-events-none z-10">
                    <button
                      onClick={() => navigateMasteredSlideshow("prev")}
                      disabled={masteredSlideshowIndex <= 0}
                    ...
                    <button
                      onClick={() => navigateMasteredSlideshow("next")}
                      disabled={
                        masteredSlideshowIndex >= masteredSlideshow.length - 1
                      }
                  ...
                  <div className="w-full mt-4 mb-8">
                    <div
                      className="w-full h-[520px] perspective-1000 cursor-pointer"
                      onClick={() => setMasteredRevealed(!masteredRevealed)}
                    >
                      {renderCardFaces(
                        masteredDisplayedCard,
                        masteredRevealed,
                        <div className="flex items-center gap-2 opacity-60">
                          <Icon name="touch_app" size="sm" />
                          <span className="text-sm">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Icon
                        name={masteredRevealed ? "rotate_left" : "visibility"}
                        size="lg"
                      />
                ...
                <div className="w-full max-w-4xl animate-fade-in">
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                      <div className="size-12 rounded-2xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 border border-emerald-500/30">
                      ...
                      <div>
                        ...
                        <p className="text-text-secondary text-sm">
                          Tổng cộng {masteredCards.length} từ đã học
                        </p>
                      </div>
                    </div>
                    <Button
                      ...
                      disabled={isLoadingMastered}
                    >
                      <Icon
                        name="refresh"
                        size="sm"
                        className={isLoadingMastered ? "animate-spin" : ""}
                      />
                    </Button>
                  </div>
                  {isLoadingMastered ? (
                    ...
                    <div className="space-y-8">
                      {hskGroups.map((hsk) => {
                        ...
                        return (
                          <div key={hsk} className="space-y-4">
                            <div className="flex items-center gap-3">
                              <span className="px-3 py-1 bg-primary/20 text-primary text-xs font-black rounded-lg border border-primary/30">
                                HSK {hsk}
                              </span>
                              <div className="h-px flex-1 bg-gradient-to-r from-border-color to-transparent" />
                              <button
                                onClick={() => enterMasteredSlideshow(cards)}
                                className="flex items-center gap-1 text-primary text-xs font-bold group"
                              >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                              {cards.map((card) => (
                                <div
                                  key={card.id}
                                  className="bg-surface-dark border border-border-color p-4 rounded-2xl hover:border-primary/50 transition-all group flex flex-col gap-2 cursor-pointer"
                                  onClick={() => enterMasteredSlideshow([card])}
                                >
                                  <div className="flex justify-between items-start">
                                    <span className="text-2xl font-chinese text-text-base group-hover:text-primary transition-colors">
                                      {getVocab(card)?.hanzi}
                                    </span>
                                    <SpeakerButton
                                      text={getVocab(card)?.hanzi || ""}
                                      size="sm"
                                    />
                                  </div>
                                  <span className="text-primary/70 text-xs font-pinyin">
                                    {getVocab(card)?.pinyin ||
                                      card.sourcePinyin}
                                  </span>
                                  <span className="text-text-secondary text-xs truncate italic">
                                    {getVocab(card)?.meaningVi}
                                  </span>
                                </div>
                              ))}
                            </div>
                      ...
                      {otherCards.length > 0 && (
                        <div className="space-y-4">
                          <div className="flex items-center gap-3">
                            <span className="px-3 py-1 bg-gray-500/20 text-gray-400 text-xs font-black rounded-lg border border-gray-500/30">
                              Khác
                            </span>
                            <div className="h-px flex-1 bg-gradient-to-r from-border-color to-transparent" />
                            <button
                              onClick={() => enterMasteredSlideshow(otherCards)}
                              className="flex items-center gap-1 text-primary text-xs font-bold group"
                            >
                              <Icon name="style" size="sm" />
                              <span className="group-hover:underline">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                            {otherCards.map((card) => (
                              <div
                                key={card.id}
                                className="bg-white/50 dark:bg-surface-dark border border-border-color p-4 rounded-2xl hover:border-primary/50 transition-all group flex flex-col gap-2 cursor-pointer"
                                onClick={() => enterMasteredSlideshow([card])}
                              >
                                <div className="flex justify-between items-start">
                                  <span className="text-2xl font-chinese text-text-base group-hover:text-primary transition-colors">
                                ...
                                <span className="text-primary/70 text-xs font-pinyin">
                                  {getVocab(card)?.pinyin || card.sourcePinyin}
                                </span>
                                <span className="text-text-secondary text-xs truncate italic">
                                  {getVocab(card)?.meaningVi}
                                </span>
            ...
            viewMode === "review" ? (
              reviewCompleted ? (
                <div className="text-center max-w-md animate-fade-in">
                  <div className="size-24 mx-auto mb-6 rounded-full bg-orange-500/20 flex items-center justify-center">
                    <Icon
                      name="emoji_events"
                  ...
                  <p className="text-text-secondary mb-6">
                    Bạn đã ôn xong {reviewHistory.length} từ vựng.
                  </p>
                ...
                <div className="text-center max-w-md animate-fade-in">
                  <div className="size-24 mx-auto mb-6 rounded-full bg-orange-500/20 flex items-center justify-center">
                    <Icon name="inbox" className="text-5xl text-orange-400" />
                  </div>
                  <h2 className="text-2xl font-bold text-text-base mb-2">
                    Chưa có từ để ôn tập
                  </h2>
                  <p className="text-text-secondary mb-6">
                    Hãy học trong &quot;Cần học&quot; trước để có từ vựng ôn
                    tập.
                  </p>
                  <Button variant="primary" onClick={() => setViewMode("srs")}>
                    Về Cần học
                  </Button>
                </div>
              ) : (
            ...
            srsQueueEmpty ? (
              <div className="text-center max-w-md animate-fade-in flex flex-col items-center justify-center flex-1">
                <div className="size-24 mx-auto mb-6 rounded-full bg-primary/20 flex items-center justify-center">
                  <Icon
                    name={reviewedCount > 0 ? "celebration" : "inbox"}
                ...
                <div className="flex flex-col gap-3 w-full max-w-xs">
                  ...
                  >
                    <Icon name="grid_view" size="sm" className="mr-2" />
                    Ghép đôi
                  </Button>
                </div>
              </div>
