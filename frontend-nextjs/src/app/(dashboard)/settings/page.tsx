...
export default function SettingsPage() {
  ...
  const handleDailyGoalChange = (value: number) => {
    setDailyGoal(value);
    setSettingsChanged(
      value !== user.dailyGoalMinutes || hskLevel !== user.hskLevel,
    );
  };
  ...
  const handleHskLevelChange = (value: number) => {
    setHskLevel(value);
    setSettingsChanged(
      dailyGoal !== user.dailyGoalMinutes || value !== user.hskLevel,
    );
  };
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto px-4 py-6 sm:p-6">
        <div className="flex items-center gap-3 mb-6 sm:mb-8">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-text-base mb-0 uppercase tracking-wider opacity-80">
            Cài đặt
          </h1>
        </div>
        ...
        <div className="bg-surface-dark rounded-3xl border border-border-color p-6 sm:p-8 mb-6 shadow-sm transition-colors">
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {THEME_MODE_OPTIONS.map((mode) => (
                  ...
                  >
                    ...
                    />
                    <span
                      className={`text-xs ${themeMode === mode.id ? "text-primary font-black" : "text-text-secondary font-medium"}`}
                    >
                      {mode.name}
                    </span>
                  </button>
            ...
            <div
              className={
                themeMode === "light" ? "opacity-50 pointer-events-none" : ""
              }
            >
              <label className="block text-sm font-medium text-text-secondary mb-3">
                Màu chủ đạo{" "}
                ...
                {presets.map((preset) => (
                  <button
                    ...
                    className={`group flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all ${
                      themeConfig.primaryMode === "preset" &&
              ...
              <div className="flex items-center gap-3 mt-4 p-3 bg-background-dark/50 rounded-2xl border border-border-color/30">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-text-base text-sm font-bold truncate">
                    Màu tùy chỉnh
                  </p>
                  <p className="text-text-secondary text-[10px] uppercase font-mono">
                    {currentPrimaryColor}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-3">
                {backgroundPresets.map((preset) => (
                  <button
                    ...
                    className={`group flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all ${
                      themeConfig.backgroundMode === "preset" &&
                      themeConfig.backgroundPresetId === preset.id
                        ? "border-primary bg-primary/10"
                        : "border-border-color hover:border-primary/50"
                    }`}
                    title={preset.name}
            ...
            <div>
              ...
              {showPasswordForm && (
                <form
                  ...
                  className="space-y-4 mt-6 p-4 sm:p-6 bg-background-dark rounded-2xl border border-border-color/50"
                >
                  {passwordError && (
