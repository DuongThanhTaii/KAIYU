...
export default function DashboardPage() {
  ...
  const [vocabStats, setVocabStats] = useState<UserVocabularyStats | null>(
    null,
  );
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const [
        ...
        videoApi.getRecommendations("dashboard", 3).catch(() => null),
      ]);
      ...
      const sortedByViews = allVideos.data.sort(
      ...
      if (sortedByViews.length > 0) {
        ...
        const randomTop =
          topVideos[Math.floor(Math.random() * topVideos.length)];
      ...
      if (user && vocab) {
        ...
        const hskTargets: Record<number, number> = {
          1: 150,
          2: 300,
          3: 600,
          4: 1200,
        ...
        const vocabPercent = Math.min(
          100,
          Math.round((vocab.total / target) * 100),
        );
  ...
  const resumeVideo = recentProgress.find(
    (p) => p.progressPercent > 0 && p.progressPercent < 100,
  );
  const nextUpVideo = recommendations?.nextUp?.video || null;
  const heroVideo =
    nextUpVideo || resumeVideo?.video || randomVideos[0] || null;
  const getVideoThumbnail = (video?: {
    thumbnailUrl?: string | null;
    videoUrl?: string;
  }) => {
    if (!video) return "/placeholder-video.jpg";
  ...
  const dailyGoalPercent = Math.min(
    100,
    Math.round((minutesStudied / (user?.dailyGoalMinutes || 30)) * 100),
  );
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col gap-6 pb-8">
        {/* Welcome Header */}
        <div>
          <p className="text-3xl md:text-4xl font-black text-text-base tracking-tight mb-1">
            Xin chào, {user?.name || "Bạn"}! 👋
          </p>
          <p className="text-text-secondary text-base md:text-lg font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 auto-rows-min">
          ...
          <Card
            variant="default"
            padding="none"
            className="col-span-1 md:col-span-2 xl:col-span-2 relative overflow-hidden group cursor-pointer min-h-[280px] border border-white/10"
            onClick={() =>
              nextUpVideo
                ? router.push(`/learn/${nextUpVideo.id}`)
                : resumeVideo
                  ? router.push(`/learn/${resumeVideo.videoId}`)
                  : router.push("/learn")
            }
          >
            ...
            <div className="absolute inset-0">
              {heroVideo ? (
                <div
                  ...
                  style={{
                    backgroundImage: `url(${getVideoThumbnail(heroVideo)})`,
                  }}
                />
              ) : (
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-background-dark/95 via-background-dark/65 to-background-dark/20" />
            ...
            <div className="relative p-5 md:p-6 flex flex-col justify-end h-full">
              <div className="flex items-center gap-3 mb-3">
                ...
                {resumeVideo && !nextUpVideo && (
                  <span className="text-primary text-sm font-bold flex items-center gap-1">
                    <Icon name="schedule" size="sm" />
                    {Math.round(
                      ((100 - resumeVideo.progressPercent) *
                        resumeVideo.video.durationSeconds) /
                        100 /
                        60,
                    )}{" "}
                    phút còn lại
              ...
              <h3 className="text-2xl md:text-[32px] font-black text-text-base mb-2 leading-tight">
                {heroVideo?.title || "Khám phá thư viện video"}
              </h3>
              <p className="text-text-secondary text-sm md:text-base mb-5 font-bold line-clamp-2 max-w-lg">
                {nextUpVideo && recommendations?.nextUp ? (
                  recommendations.nextUp.reasons?.join(" • ")
                ) : resumeVideo ? (
                  <span>
                    HSK {resumeVideo.video.hskLevel} •{" "}
              ...
              <div className="flex items-center gap-3 bg-black/30 backdrop-blur-sm p-2 rounded-full pr-5 w-fit border border-white/10">
                <button className="bg-primary text-on-primary rounded-full size-9 flex items-center justify-center hover:scale-105 transition-transform">
          ...
          <Card
            variant="default"
            className="col-span-1 md:col-span-2 xl:col-span-2 p-4 md:p-5 border border-white/10 bg-surface-dark/40"
            padding="none"
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-black text-text-base">
                Gợi ý hôm nay
              </h3>
            </div>
            {(recommendations?.suited?.length || 0) > 0 ? (
              <div className="space-y-2.5">
                {recommendations!.suited.slice(0, 3).map((item) => (
                  <Link
                    ...
                    className="flex items-center gap-3 rounded-xl border border-white/10 bg-background-dark/35 p-2.5 hover:border-primary/50 hover:bg-background-dark/55 transition-all"
                  >
                    <div
                      className="w-[96px] h-[56px] rounded-lg bg-cover bg-center shrink-0"
                      style={{
                        backgroundImage: `url(${getVideoThumbnail(item.video)})`,
                      }}
                    />
                    <div className="min-w-0">
                      <p className="text-sm font-black text-text-base line-clamp-1">
                        {item.video.title}
                      </p>
              ...
              <div className="rounded-2xl border border-dashed border-primary/30 bg-primary/5 p-4 md:p-5">
                <p className="text-sm text-text-secondary">
                  Chưa đủ dữ liệu đề xuất. Hãy học 1-2 video HSK{" "}
                  {user?.hskLevel || 1} để hệ thống tối ưu lộ trình.
                </p>
                <div className="mt-3">
                  <Button
                    variant="secondary"
          ...
          <section className="col-span-1 md:col-span-2 lg:col-span-3 xl:col-span-4 grid grid-cols-1 xl:grid-cols-[0.67fr_0.67fr_1fr] gap-4 items-stretch">
            <div className="grid grid-rows-[1fr_1fr] gap-4 h-[440px]">
          {/* 2. SRS Review Today */}
          <Card
            variant="default"
            hover
            className="border border-white/10 bg-surface-dark/40 p-4 md:p-5 flex flex-col justify-between"
            padding="none"
          >
            <div>
          ...
          <Card
            variant="default"
            className="border border-white/10 bg-surface-dark/40 p-4 md:p-5 flex flex-col justify-center items-center"
            padding="none"
          >
            <div className="relative size-24 mb-3">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-xl font-black text-text-base">
                  {dailyGoalPercent}%
                </span>
              </div>
            </div>
            <p className="text-sm font-semibold text-text-base">Mục tiêu hôm nay</p>
          ...
          <Card
            variant="default"
            padding="none"
            className="flex flex-col overflow-hidden border border-white/10 bg-surface-dark/40 h-[440px]"
          >
            <div className="p-4 md:p-5 border-b border-border-color">
            ...
            <div className="flex-1 p-3 md:p-3 flex flex-col gap-2.5 overflow-y-auto">
              <div className="flex items-center justify-between p-3 bg-surface-dark/50 rounded-xl border border-white/10">
                <span className="text-text-secondary font-bold text-sm md:text-base">
                  Tổng đã lưu
                </span>
                <span className="text-xl md:text-2xl font-black text-text-base">
                  {vocabStats?.total || 0}
                </span>
              </div>
              <div className="flex items-center justify-between p-3 bg-surface-dark/50 rounded-xl border border-white/10">
              ...
              <div className="flex items-center justify-between p-3 bg-surface-dark/50 rounded-xl border border-white/10">
                ...
                <span className="text-xl md:text-2xl font-bold text-orange-600 dark:text-orange-400">
                  {(vocabStats?.review || 0) + (vocabStats?.new || 0)}
                </span>
              </div>
            </div>
          ...
          <Card
            variant="default"
            className="p-4 md:p-6 border border-white/10 bg-surface-dark/40 h-[440px]"
            padding="none"
          >
              ...
              {weeklyProgress?.days?.map((dayData, i) => {
                const maxMinutes = Math.max(
                  ...(weeklyProgress?.days?.map(
                    (d) => (d.vocabularySaved || 0) + (d.reviewsCompleted || 0),
                  ) || [1]),
                  1,
                );
                const activity =
                  (dayData.vocabularySaved || 0) +
                  (dayData.reviewsCompleted || 0);
                const height =
                  maxMinutes > 0 ? (activity / maxMinutes) * 100 : 0;
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] uppercase font-bold ${isToday ? "text-primary" : "text-text-secondary"}`}
                    >
                      {dayLabels[dayOfWeek]}
                    </span>
                  </div>
          ...
          <Card
            variant="default"
            padding="none"
            className="col-span-1 md:col-span-2 xl:col-span-4 overflow-hidden"
          >
            ...
            {achievements.length === 0 ? (
              <div className="p-8 text-center">
                <Icon
                  name="emoji_events"
                  className="text-4xl text-text-secondary mb-2"
                />
                <p className="text-text-secondary">
                  Chưa có thành tựu nào. Hãy tiếp tục học để đạt được!
                </p>
              </div>
            ) : (
              <>
                      ...
                      {achievements.slice(0, 5).map((item) => (
                        ...
                        >
                          <td className="px-6 py-4 font-medium text-text-base flex items-center gap-3">
                            <div
                              className={`size-8 rounded-full bg-${item.iconColor?.replace("text-", "")}/20 flex items-center justify-center ${item.iconColor || "text-primary"}`}
                            >
                              <Icon
                                name={item.icon || "emoji_events"}
                                size="md"
                              />
                            </div>
                            {item.title}
