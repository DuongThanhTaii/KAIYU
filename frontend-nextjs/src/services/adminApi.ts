...
export const getAnalyticsPermissions = (): Promise<AnalyticsPermissions> => {
  return apiRequest<AnalyticsPermissions>("/admin/analytics/permissions");
};
...
export const getAllVideos = (
  ...
  return apiRequest<PaginatedResponse<Video>>(
...
export const getAllVocabulary = (
  ...
  const query = searchParams.toString();
  return apiRequest<PaginatedResponse<Vocabulary>>(
    `/admin/vocabulary${query ? `?${query}` : ""}`,
  );
};
...
export const validateImport = (
  ...
  return apiRequest<{ duplicates: string[]; total: number }>(
    "/vocabulary/validate-import",
    {
      method: "POST",
...
export const bulkUpdateVocabulary = (
  ...
  return apiRequest<{
    ...
    errors: number;
    errorDetails?: { hanzi: string; error: string }[];
  }>("/vocabulary/bulk-update", {
    method: "POST",
    body: JSON.stringify(items),
  });
};

export const getVocabularyStats = (): Promise<
  { hskLevel: number; count: number }[]
> => {
  return apiRequest<{ hskLevel: number; count: number }[]>("/vocabulary/stats");
};

export const deleteAllVocabulary = (): Promise<{
  message: string;
  deleted: number;
}> => {
...
export const getAllAchievements = (
  ...
  const query = searchParams.toString();
  return apiRequest<PaginatedResponse<Achievement>>(
    `/admin/achievements${query ? `?${query}` : ""}`,
  );
};
...
// ============ Upload Status ============
export const getUploadStatus = (): Promise<{
  s3Configured: boolean;
  message: string;
}> => {
  return apiRequest<{ s3Configured: boolean; message: string }>(
    "/upload/status",
  );
};
