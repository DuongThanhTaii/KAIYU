...
interface SpeechRecognitionEventLike {
  results: ArrayLike<ArrayLike<SpeechRecognitionResultAlternativeLike>>;
}
...
export const transcribeAudio = async (
  ...
  if (whisperUrl) {
    try {
      ...
      console.warn(
        "[STT] Whisper API failed, falling back to Web Speech API:",
        error,
      );
    }
  }
...
export const assessPronunciation = (
  ...
  const completeness =
    reference.length > 0
      ? Math.round(
          (Math.min(spoken.length, reference.length) / reference.length) * 100,
        )
      : 0;
  ...
  const overall = Math.min(
    100,
    Math.round(accuracy * 0.7 + completeness * 0.3 + lengthBonus),
  );
