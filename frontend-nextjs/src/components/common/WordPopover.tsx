...
import {
  vocabularyFoldersApi,
  type VocabularyFolder,
} from "@/services/vocabularyFoldersApi";
...
import {
  renderGroupedPinyin,
  renderFormattedMeaning,
  highlightWord,
} from "@/utils/chinese";
import { checkSequenceConflict } from "@/utils/subtitle";
import { reorderTokens } from "@/utils/array";
...
export function WordPopover({
  ...
  const [enrichedData, setEnrichedData] = useState<EnrichedWordData | null>(
    null,
  );
  ...
  const [draggingTokenIndex, setDraggingTokenIndex] = useState<number | null>(
    null,
  );
  const [showSequenceConfirmModal, setShowSequenceConfirmModal] =
    useState(false);
  ...
  const [notFoundMode, setNotFoundMode] = useState<
    "info" | "search" | "create"
  >("info");
  ...
  const [lookupHistory, setLookupHistory] = useState<
    { word: string; pinyin: string; meaning: string; timestamp: number }[]
  >([]);
  ...
  const syncEditedWordToTokens = useCallback(
    (meaning: string, pinyin?: string) => {
      const targetHanzi = (editHanzi || word).trim();
      if (!targetHanzi) return;

      setEditingTokens((prev) =>
        prev.map((t) => {
          if (t.hanzi !== targetHanzi) return t;

          return {
            ...t,
            meaning,
            ...(typeof pinyin === "string" ? { pinyin } : {}),
          };
        }),
      );
    },
  ...
  const originalSubtitleSentence = (
    sourceSentence ||
    subtitle?.hanzi ||
    ""
  ).trim();
  const currentEditedSentence = editingTokens
    .map((t: any) => (typeof t.hanzi === "string" ? t.hanzi : ""))
    .join("");
  const hasSequenceConflict =
    editingTokens.length > 0 &&
    checkSequenceConflict(editingTokens, originalSubtitleSentence, false);

  const remapSelectedTokenIndex = useCallback(
    (
  ...
  const handleTokenDragStart = (
    ...
    setDraggingTokenIndex(index);
  };

  const handleTokenDragOver = (event: DragEvent<HTMLDivElement>) => {
    event.preventDefault();
    event.dataTransfer.dropEffect = "move";
  };

  const handleTokenDrop = (targetIndex: number) => {
    if (draggingTokenIndex === null || draggingTokenIndex === targetIndex) {
    ...
    setEditingTokens((prev) =>
      reorderTokens(
        prev as Array<{ position: number } & Record<string, any>>,
        draggingTokenIndex,
        targetIndex,
      ),
    );
    setSelectedTokenIndex((prev) =>
      remapSelectedTokenIndex(prev, draggingTokenIndex, targetIndex),
    );
    setDraggingTokenIndex(null);
  };
    ...
    if (needsEnrich && hasEnrichedRef.current !== word && lookupResult?.found) {
      ...
      dictionaryApi
        .getEnrichedData(
          word,
          lookupResult.pinyin,
          lookupResult.meaningVi || lookupResult.meaningEn,
        )
  ...
  const handleSaveGlobal = async () => {
    const normalizedHanzi = (editHanzi || word).trim();
    const normalizedPinyin = editPinyin.trim();
    const normalizedMeaning = editMeaning.trim();
    const normalizedPos = editPos.trim();

    if (!normalizedHanzi || !normalizedMeaning) return;
    ...
    try {
      ...
      // Always clear cache so the follow-up lookup cannot return stale data
      await dictionaryApi.clearCache();

      // Re-lookup to get fresh data (especially if newly created)
      const freshResult = await dictionaryApi.lookup(
        normalizedHanzi,
        sourcePinyin,
      );
      setLookupResult(freshResult);
      if (freshResult.found) {
        setEditHanzi(freshResult.hanzi || normalizedHanzi);
        setEditPinyin(freshResult.pinyin || normalizedPinyin);
        setEditMeaning(
        ...
        syncEditedWordToTokens(normalizedMeaning, normalizedPinyin);
      }
  ...
  const handleSaveSubtitle = async (forceSaveOnConflict = false) => {
    if (!subtitle) return;
    if (editingTokens.length === 0) return;

    const currentFullHanzi = editingTokens
      .map((t: any) => (typeof t.hanzi === "string" ? t.hanzi : ""))
      .join("");
    const currentFullPinyin = editingTokens
      .map((t: any) => (typeof t.pinyin === "string" ? t.pinyin.trim() : ""))
      .filter(Boolean)
      .join(" ");

    const shouldConfirmSequence = checkSequenceConflict(
      editingTokens,
      originalSubtitleSentence,
      forceSaveOnConflict,
    );
    if (shouldConfirmSequence) {
      setShowSequenceConfirmModal(true);
      return;
    }
    ...
    <>
      ...
      >
        ...
        <div className="relative bg-[var(--color-surface-dark)]/95 backdrop-blur-xl border border-border-color rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <>
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="flex items-start justify-between gap-6">
                    <div className="min-w-0 flex-1">
                      {/* Large Hanzi */}
                      <h2
                        className={`font-bold text-text-base font-chinese leading-[1.1] whitespace-nowrap overflow-hidden text-ellipsis ${word.length > 4 ? "text-3xl md:text-4xl" : "text-4xl md:text-5xl"}`}
                        lang="zh-CN"
                      >
                        {word}
                      </h2>
                    ...
                    <div className="flex items-center bg-[var(--color-surface-dark)]/50 border border-border-color/30 rounded-2xl p-1 shrink-0 backdrop-blur-md">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className={`inline-flex items-center justify-center p-2 rounded-full transition-all hover:scale-110 active:scale-95 ${
                            activePanel === "admin_edit"
                      ...
                      {lookupResult.isSystemWord && (
                        ...
                        >
                          {isSaving ? (
                            ...
                            <Icon
                              name={saveSuccess ? "check" : "bookmark_add"}
                              size="sm"
                            />
                          )}
                        </button>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {lookupResult.isSystemWord && lookupResult.hskLevel ? (
                      <div
                        className={`h-8 px-3 ${HSK_COLORS[lookupResult.hskLevel] || "bg-gray-500"} bg-opacity-20 text-text-base text-[10px] font-black rounded-full uppercase tracking-wider inline-flex items-center gap-1.5 shadow-sm leading-none whitespace-nowrap`}
                      >
                        <span className="opacity-70 text-[8px] font-bold">
                          LEVEL
                        </span>
                    ...
                    {lookupResult.partOfSpeech && (
                      <div className="h-8 px-3 bg-surface-highlight border border-border-color/50 text-text-secondary text-[10px] font-black rounded-full uppercase tracking-wider inline-flex items-center gap-1.5 leading-none whitespace-nowrap">
                        <Icon
                          name="category"
                          size="sm"
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar">
                ...
                {lookupResult.meaningVi && (
                  <div
                    className="px-5 py-3 bg-surface-highlight/5 border-b border-border-color/30 animate-in fade-in slide-in-from-bottom-2 duration-300"
                    lang="vi"
                  >
                      ...
                      {lookupResult.meaningVi.includes("1.") ? (
                        lookupResult.meaningVi
                          ...
                          .map((part, i) => (
                            <div
                              key={i}
                              className="text-lg font-medium leading-relaxed group transition-colors hover:text-primary"
                            >
                      ...
                      {examples.length > 0 && (
                        <div className="pt-3 border-t border-border-color space-y-4">
                          {examples.slice(0, 3).map((ex, i) => (
                            <div
                              key={i}
                              className="flex flex-col gap-2 p-4 bg-surface-highlight/20 border border-border-color/30 rounded-2xl group hover:bg-surface-highlight/40 transition-all"
                            >
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest opacity-80">
                                  Ví dụ {i + 1}
                                </span>
                                <SpeakerButton
                                  text={ex.chinese}
                              ...
                              <div className="space-y-1">
                                <p
                                  className="text-text-base font-chinese text-lg leading-snug"
                                  lang="zh-CN"
                                >
                        ...
                        <>
                          ...
                          {enrichedData?.decomposition?.radical?.meaning && (
                            <div className="bg-surface-highlight/50 rounded-xl p-4">
                              ...
                              <div className="flex items-center gap-3">
                                <span
                                  className="text-3xl font-chinese text-text-base"
                                  lang="zh-CN"
                                >
                                ...
                                <div>
                                  ...
                                  {enrichedData.decomposition.radical
                                    .pinyin && (
                          ...
                          {enrichedData?.relatedWords && (
                            <div className="grid grid-cols-2 gap-3">
                              {enrichedData.relatedWords.synonyms?.length >
                                0 && (
                                <div>
                                  ...
                                  <div className="flex flex-col gap-1.5">
                                    {enrichedData.relatedWords.synonyms
                                      .slice(0, 4)
                                      .map((w, i) => (
                                        <div
                                          key={i}
                                          className="px-3 py-2 bg-emerald-500/5 border border-emerald-500/20 rounded-xl group hover:bg-emerald-500/10 transition-colors"
                                        >
                                          <div className="flex items-baseline justify-between gap-2">
                                            <span
                                              className="text-emerald-400 font-chinese text-sm font-bold whitespace-nowrap"
                                              lang="zh-CN"
                                            >
                                              {w.hanzi}
                                            </span>
                                            <span className="text-emerald-400/60 text-[10px] truncate uppercase font-bold tracking-tighter">
                                              {w.meaning}
                                            </span>
                                          </div>
                                        </div>
                              ...
                              {enrichedData.relatedWords.antonyms?.length >
                                0 && (
                                <div>
                                  ...
                                  <div className="flex flex-col gap-1.5">
                                    {enrichedData.relatedWords.antonyms
                                      .slice(0, 4)
                                      .map((w, i) => (
                                        <div
                                          key={i}
                                          className="px-3 py-2 bg-rose-500/5 border border-rose-500/20 rounded-xl group hover:bg-rose-500/10 transition-colors"
                                        >
                                          <div className="flex items-baseline justify-between gap-2">
                                            <span
                                              className="text-rose-400 font-chinese text-sm font-bold whitespace-nowrap"
                                              lang="zh-CN"
                                            >
                                              {w.hanzi}
                                            </span>
                                            <span className="text-rose-400/60 text-[10px] truncate uppercase font-bold tracking-tighter">
                                              {w.meaning}
                                            </span>
                                          </div>
                                        </div>
                          ...
                          {/* Collocations */}
                          {(enrichedData?.relatedWords?.collocations?.length ??
                            0) > 0 && (
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {enrichedData?.relatedWords?.collocations?.map(
                                  (w, i) => (
                                    <div
                                      key={i}
                                      className="px-3 py-1.5 bg-surface-highlight border border-border-color/30 rounded-xl flex items-center gap-2"
                                    >
                                      <span
                                        className="text-text-base font-chinese whitespace-nowrap"
                                        lang="zh-CN"
                                      >
                                        {w.hanzi}
                                      </span>
                                      <span className="text-text-secondary text-[10px] font-medium leading-none">
                                        ({w.meaning})
                                      </span>
                                    </div>
                                  ),
                    ...
                    <div className="p-4 space-y-5">
                      ...
                      {sourceSentence ? (
                        <div className="bg-primary/5 rounded-2xl p-4 border border-primary/20 space-y-2 animate-in fade-in slide-in-from-top-2">
                          ...
                          <p
                            className="text-text-base font-chinese text-base leading-relaxed"
                            lang="zh-CN"
                          >
                            {highlightWord(sourceSentence, word)}
                          </p>
                          {sourcePinyin && (
                            <p className="text-text-secondary text-xs font-pinyin tracking-tight opacity-70 italic line-clamp-1">
                              {sourcePinyin}
                            </p>
                          )}
                        </div>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] text-text-secondary uppercase tracking-widest font-black flex items-center gap-2">
                          <Icon name="sticky_note_2" size="sm" /> Ghi chú cá
                          nhân
                        </label>
                        <textarea
                          value={note}
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <h4 className="text-[10px] text-text-secondary uppercase tracking-widest font-black">
                            Chọn thư mục
                          </h4>
                          <button
                            onClick={() =>
                        ...
                        >
                          {saveSuccess ? (
                            ...
                            <div className="flex items-center justify-center gap-3">
                              <span className="font-bold">
                                Lưu vào bộ nhớ{" "}
                                <span className="text-black/60 opacity-80">
                                  (+10 XP)
                                </span>
                              </span>
                            </div>
                      ...
                      {adminSubTab === "word" ? (
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-1">
                          <div className="bg-surface-highlight/20 p-3 rounded-xl border border-border-color/30">
                            <h4 className="text-[10px] text-text-secondary uppercase tracking-wider mb-3">
                              Kịch bản phân đoạn hiện tại
                            </h4>
                            <div className="flex flex-wrap gap-2">
                              {editingTokens.map((token, idx) => (
                                ...
                                >
                                  <div
                                    draggable
                                    onDragStart={(event) =>
                                      handleTokenDragStart(event, idx)
                                    }
                                    onDragOver={handleTokenDragOver}
                                    onDrop={() => handleTokenDrop(idx)}
                                    onDragEnd={handleTokenDragEnd}
                                    onClick={() => setSelectedTokenIndex(idx)}
                                    className={`flex items-center bg-primary/10 border ${selectedTokenIndex === idx ? "border-primary ring-1 ring-primary" : "border-primary/30"} ${draggingTokenIndex === idx ? "opacity-50 scale-95" : "opacity-100"} rounded-lg px-3 py-2 hover:border-primary transition-all cursor-grab active:cursor-grabbing`}
                                  >
                                    <Icon
                                      name="drag_indicator"
                                      size="sm"
                                    ...
                                    />
                                    <span className="text-lg font-chinese text-primary">
                                      {token.hanzi}
                                    </span>
                                    <div className="ml-2 flex flex-col opacity-0 group-hover:opacity-100 transition-opacity">
                                      {token.hanzi.length > 1 && (
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            handleSplitToken(idx);
                                          }}
                                          className="p-0.5 hover:text-text-base text-text-secondary"
                                          title="Split"
                                        >
                          ...
                          {selectedTokenIndex !== null &&
                            editingTokens[selectedTokenIndex] && (
                              <div className="p-3 bg-surface-highlight/20 border border-border-color rounded-xl space-y-3 animate-in fade-in zoom-in-95 duration-200">
                                <div className="flex items-center justify-between">
                                  <h5 className="text-xs text-primary font-bold">
                                    Chỉnh sửa:{" "}
                                    {editingTokens[selectedTokenIndex].hanzi}
                                  </h5>
                                  <button
                                  ...
                                  <div>
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const newTokens = [...editingTokens];
                                        const val = e.target.value;
                                        newTokens[selectedTokenIndex].pinyin =
                                          val;
                                        setEditingTokens(newTokens);
                                        if (
                                          newTokens[selectedTokenIndex]
                                            .hanzi === word
                                        ) {
                                          setEditPinyin(val);
                                        }
                                  ...
                                  <div>
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const newTokens = [...editingTokens];
                                        const val = e.target.value;
                                        newTokens[selectedTokenIndex].meaning =
                                          val;
                                        setEditingTokens(newTokens);
                                        if (
                                          newTokens[selectedTokenIndex]
                          ...
                          <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-xl">
                            <p className="text-[10px] text-amber-300 leading-relaxed italic">
                              Thay đổi ở tab này chỉ áp dụng cho video này. Hệ
                              thống sẽ bỏ qua bộ phân đoạn tự động.
                            </p>
                          </div>

                          {hasSequenceConflict && (
                            <div className="p-3 bg-rose-500/10 border border-rose-500/30 rounded-xl">
                              <p className="text-[10px] text-rose-300 font-semibold uppercase tracking-wider mb-1">
                                Cảnh báo lệch câu gốc
                              </p>
                              <p className="text-xs text-rose-200/90">
                                Hiện tại: {currentEditedSentence || "(trống)"}
                              </p>
                              <p className="text-xs text-rose-200/90 mt-1">
                                Câu gốc:{" "}
                                {originalSubtitleSentence || "(không có)"}
                              </p>
                            </div>
                          )}
            ...
            <div className="flex-1 flex flex-col max-h-[70vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="p-5 border-b border-border-color bg-surface-highlight/10">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <h2
                      className={`font-bold text-text-base font-chinese leading-[1.1] ${word.length > 4 ? "text-4xl" : "text-5xl"}`}
                      lang="zh-CN"
                    >
                      {word}
                    </h2>
                ...
                <div className="px-5 py-4 border-b border-border-color/30">
                  <div className="flex items-center gap-2 mb-2">
                    <Icon
                      ...
                      className="text-amber-400"
                    />
                    <span className="text-[10px] text-amber-300 font-black uppercase tracking-widest">
                      Nghĩa theo ngữ cảnh video
                    </span>
                  </div>
                  <p className="text-xl text-text-base font-semibold leading-snug">
                    {matchingToken.meaning}
                  </p>
                </div>
              ) : (
                  ...
                  {activePanel === "admin_edit" ? (
                    ...
                    <div className="p-4 space-y-4">
                          ...
                          {searchResults.length > 0 ? (
                            <div className="space-y-1.5 max-h-[200px] overflow-y-auto custom-scrollbar-thin pr-1">
                              {searchResults.map((result, idx) => (
                                ...
                                >
                                  <span
                                    className="text-xl font-chinese text-text-base group-hover:text-primary transition-colors"
                                    lang="zh-CN"
                                  >
                                    {result.hanzi}
                                  </span>
                                  <div className="flex-1 min-w-0">
                                    <p className="text-primary text-sm font-pinyin tracking-tight">
                                      {result.pinyinDisplay}
                                    </p>
                                    <p className="text-text-secondary text-xs truncate">
                                      {result.meaningVi || result.meaningEn}
                                    </p>
                                  </div>
                                  <Icon
                                    name="link"
                                    size="sm"
                                    className="text-text-secondary group-hover:text-primary opacity-0 group-hover:opacity-100 transition-all"
                                  />
                                </button>
                              ))}
                            </div>
                            ...
                            <div className="text-center py-6">
                              <Icon
                                name="search_off"
                                className="text-3xl text-text-secondary/50 mb-2"
                              />
                            ...
                            <div className="text-center py-4">
                              <p className="text-text-secondary/60 text-xs">
                                Nhập từ hoặc pinyin để tìm kiếm
                              </p>
                            </div>
                          ) : null}
                        </div>
                          ...
                          {createSuccess ? (
                            <div className="text-center py-6">
                              ...
                              <p className="text-emerald-400 font-bold">
                                Đã tạo từ thành công!
                              </p>
                              <p className="text-text-secondary text-xs mt-1">
                                Từ đã được thêm vào từ điển
                              </p>
                            </div>
                          ) : (
                            <>
                              ...
                              <div>
                                ...
                                <input
                                  value={newWordPinyin}
                                  onChange={(e) =>
                                    setNewWordPinyin(e.target.value)
                                  }
                                  placeholder="e.g. nǐ hǎo"
                                  className="w-full bg-[var(--color-surface-dark)] border border-border-color rounded-lg px-3 py-2 text-primary focus:outline-none focus:border-primary font-pinyin"
                                  autoFocus
                              ...
                              <div>
                                ...
                                <textarea
                                  ...
                                  onChange={(e) =>
                                    setNewWordMeaning(e.target.value)
                                  }
                                  rows={2}
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                  ...
                                  <select
                                    value={newWordHsk}
                                    onChange={(e) =>
                                      setNewWordHsk(Number(e.target.value))
                                    }
                      ...
                      {notFoundMode === "info" && (
                        <div className="text-center py-4">
                          <p className="text-text-secondary/60 text-xs">
                            Chọn một hành động ở trên để thêm nghĩa cho từ này
                          </p>
                        </div>
                      )}
                    </div>
          ...
          {showSequenceConfirmModal && (
            <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="w-full max-w-md bg-[var(--color-surface-dark)] border border-rose-500/40 rounded-2xl p-4 space-y-3 shadow-2xl">
                <div className="flex items-center gap-2 text-rose-300">
                  <Icon name="warning" size="sm" />
                  <h4 className="text-sm font-bold uppercase tracking-wide">
                    Câu sau khi sửa khác câu gốc
                  </h4>
                </div>
                <p className="text-xs text-text-secondary">
                  Bạn đang lưu một chuỗi từ khác với câu gốc đã upload. Hãy xác
                  nhận nếu đây là thay đổi có chủ đích.
                </p>
                <div className="rounded-xl border border-border-color/60 bg-surface-highlight/20 p-3 space-y-2 text-xs">
                  <p className="text-text-secondary">Câu gốc:</p>
                  <p
                    className="text-text-base font-chinese break-all"
                    lang="zh-CN"
                  >
                    {originalSubtitleSentence || "(không có dữ liệu)"}
                  </p>
                  <p className="text-text-secondary pt-1">Câu hiện tại:</p>
                  <p
                    className="text-primary font-chinese break-all"
                    lang="zh-CN"
                  >
                    {currentEditedSentence || "(trống)"}
                  </p>
                </div>
                <div className="flex items-center gap-2 pt-1">
                  <Button
                    ...
                    onClick={() => setShowSequenceConfirmModal(false)}
                  >
                    Quay lại chỉnh sửa
                  </Button>
                  <Button
