...
export class EmailController {
  ...
  async testSendEmail(
    @CurrentUser() user: any,
    @Body()
    body: { templateCode: string; variables?: Record<string, string | number> },
  ) {
  ...
  async previewTemplate(@Body() body: { subject: string; htmlBody: string }) {
  ...
  async broadcastEmail(
    ...
    return this.emailService.broadcastEmail(
      body.subject,
      body.htmlBody,
      body.targetUsers || 'active',
    );
  }
}
