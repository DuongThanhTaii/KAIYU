...
export class WordEnrichmentService {
  ...
  private readonly HANZI_WRITER_DATA_URL =
    'https://cdn.jsdelivr.net/npm/hanzi-writer-data@2.0';
  private readonly MAX_RETRIES = 3;
  private readonly logger = new Logger(WordEnrichmentService.name);

  // In-memory cache for enriched data
  private cache: Map<string, { data: EnrichedWordData; timestamp: number }> =
    new Map();
  ...
  private async getStrokeData(hanzi: string): Promise<StrokeData | undefined> {
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.debug(`[StrokeData] No data for ${hanzi}`);
        return undefined;
      }
   ...
   */
  private async getDecomposition(
    hanzi: string,
  ): Promise<Decomposition | undefined> {
    if (hanzi.length !== 1) return undefined;
  ...
  private async generateMnemonic(
    ...
    pinyin: string,
    meaning: string,
  ): Promise<Mnemonic | undefined> {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      this.logger.warn('[Mnemonic] No API key, using fallback');
...
{"visualStory":"Chữ 好 gồm 女(nữ) + 子(con) = mẹ bế con = điều TỐT ĐẸP nhất","characterBreakdown":"女 (nữ) + 子 (con)"}`;
    ...
    try {
      this.logger.debug(`[Mnemonic] Generating for ${hanzi}...`);
      ...
      if (!response.ok) {
        this.logger.warn(`[Mnemonic] API error: ${response.status}`);
        return this.getFallbackMnemonic(hanzi, pinyin, meaning);
      }

      const data = await response.json();
      const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
      this.logger.debug(`[Mnemonic] Raw response length: ${text?.length ?? 0}`);
      ...
      if (text) {
        ...
        if (jsonMatch) {
          try {
            ...
            this.logger.debug('[Mnemonic] Parsed successfully');
            return parsed;
          } catch (e) {
            this.logger.debug('[Mnemonic] JSON parse error');
          }
        }

        // If no JSON found, extract and clean the text for visualStory
        if (cleanText.length > 10) {
          this.logger.debug('[Mnemonic] Using plain text as visualStory');

          // Extract visualStory content if partial JSON
          let story = cleanText;
          const visualStoryMatch = cleanText.match(
            /"visualStory"\s*:\s*"([^"]+)/i,
          );
          if (visualStoryMatch) {
          ...
          if (story.length > 10) {
            return {
              visualStory: story,
              characterBreakdown:
                hanzi.length > 1 ? hanzi.split('').join(' + ') : undefined,
            };
          }
        }
      }
      this.logger.warn('[Mnemonic] Could not parse, using fallback');
      return this.getFallbackMnemonic(hanzi, pinyin, meaning);
    } catch (error) {
   ...
   */
  private async generateRelatedWords(
    hanzi: string,
    pinyin: string,
    meaning: string,
  ): Promise<RelatedWords | undefined> {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      this.logger.warn('[RelatedWords] No API key');
      return undefined;
    }
...
Nếu không có, để mảng rỗng [].`;
    ...
    try {
      this.logger.debug(`[RelatedWords] Generating for ${hanzi}...`);
      const response = await this.callWithRetry(
        `${this.GEMINI_API_URL}?key=${apiKey}`,
        {
          method: 'POST',
      ...
      if (!response.ok) {
        this.logger.warn(`[RelatedWords] API error: ${response.status}`);
        return undefined;
      }

      const data = await response.json();
      const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
      this.logger.debug(`[RelatedWords] Response length: ${text?.length ?? 0}`);
      ...
      if (text) {
        ...
        if (jsonMatch) {
          ...
          try {
            ...
            this.logger.debug('[RelatedWords] Attempting JSON repair...');
            ...
            try {
              ...
              this.logger.debug('[RelatedWords] Repaired successfully');
              return repaired;
            } catch (e2) {
              this.logger.warn('[RelatedWords] Repair failed');
            }
          }
        }
      }
