...
export class DictionaryService {
    ...
    async lookup(hanzi: string, contextPinyin?: string): Promise<LookupResult> {
        ...
        try {
            ...
            const entries = this.dictionary?.getBySimplified?.(hanzi, null, { asObject: false }) ||
                this.dictionary?.getByTraditional?.(hanzi, null, { asObject: false });

            console.log(`Dictionary lookup for "${hanzi}":`, entries);

            if (entries && Array.isArray(entries) && entries.length > 0) {
                // Filter out variant entries for cleaner results
                const mainEntries = entries.filter((e: any) => !e.is_variant && e.simplified === hanzi);
                ...
                if (contextPinyin) {
                    ...
                    entriesToUse = entriesToUse.sort((a: any, b: any) => {
                        ...
                        const aMatch = aPinyin === contextLower || aPinyin.includes(contextLower);
                        const bMatch = bPinyin === contextLower || bPinyin.includes(contextLower);
                ...
                translatePromises.push(
                    Promise.all([
                        ...
                        this.translateDefinitionsToVietnamese(result.definitions.slice(0, 3))
                ...
                if (result.allEntries.length > 1) {
                    for (let i = 1; i < Math.min(result.allEntries.length, 4); i++) {
                        ...
                        translatePromises.push(
                            this.translateDefinitionsToVietnamese(entryItem.definitions.slice(0, 2))
                                .then(defsVi => {
            ...
            } else if (entries && typeof entries === 'object' && Object.keys(entries).length > 0) {
            ...
            if (!result.found) {
                ...
                for (const char of chars) {
                    const charEntries = this.dictionary?.getBySimplified?.(char, null, { asObject: false });
                    console.log(`Char lookup for "${char}":`, charEntries);
                    ...
                    } else if (charEntries && typeof charEntries === 'object' && Object.keys(charEntries).length > 0) {
                ...
                if (charResults.length > 0 && charResults.some(r => r !== '')) {
                    ...
                    result.pinyinDisplay = this.convertPinyinToDisplay(charPinyin.join(' '));
        ...
        try {
            ...
            for (const entry of entries.slice(0, limit)) {
                results.push({
                    ...
                    meaningEn: (entry.english || entry.definitions || []).slice(0, 3).join('; '),
                    traditional: entry.traditional,
                    simplified: entry.simplified,
                    partOfSpeech: this.extractPartOfSpeech(entry.english || entry.definitions || []),
                    found: true,
                });
            }
        } catch (error) {
     ...
     */
    async getExamples(hanzi: string): Promise<{ chinese: string; pinyin?: string; translation: string }[]> {
        const examples: { chinese: string; pinyin?: string; translation: string }[] = [];
        ...
        try {
            ...
            if (!response.ok) {
                console.log('Tatoeba API error:', response.status);
                return examples;
            }
            ...
            if (data.data && Array.isArray(data.data)) {
                for (const sentence of data.data.slice(0, 3)) {
                    const example: { chinese: string; pinyin?: string; translation: string } = {
            ...
            console.error('Failed to fetch examples from Tatoeba:', error);
        }
     ...
     */
    private async translateToVietnamese(text: string): Promise<string | undefined> {
        try {
            ...
            console.error('Failed to translate to Vietnamese:', error);
            return undefined;
        }
    }
     ...
     */
    private async translateDefinitionsToVietnamese(definitions: string[]): Promise<string[]> {
