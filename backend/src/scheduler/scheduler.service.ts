...
export class SchedulerService implements OnModuleInit, OnModuleDestroy {
    ...
    async checkStreakBreakReminders() {
        ...
        try {
            ...
            for (const user of usersAtRisk) {
                ...
                try {
                    await this.emailService.sendWithTemplate(user.id, 'streak_break_warning', {
                    ...
                    this.logger.error(`Failed to send streak reminder to ${user.email}: ${error.message}`);
                }
            }
    ...
    async checkInactiveUsers() {
        ...
        try {
            ...
            for (const user of inactive3Days) {
                if (user.emailSettings && !user.emailSettings.enableEngagement) continue;
            ...
            this.logger.log(`Processed ${inactive3Days.length} 3-day and ${inactive7Days.length} 7-day inactive users`);
        } catch (error: any) {
    ...
    async sendWeeklyReports() {
        ...
        try {
            ...
            for (const user of users) {
                ...
                if (user.emailSettings && !user.emailSettings.enableWeeklyReport) continue;
    ...
    async checkVocabDueReminders() {
        ...
        try {
            ...
            const usersWithDue = await this.prisma.$queryRaw<Array<{ userId: string; dueCount: number }>>`
