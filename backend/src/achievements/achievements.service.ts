...
export class AchievementsService {
    ...
    async getUserAchievements(userId: string): Promise<AchievementWithEarned[]> {
        ...
        const [vocabCount, flashcardReviewCount, videoCount, user] = await Promise.all([
     ...
     */
    async checkAndAwardAchievements(userId: string): Promise<AchievementWithEarned[]> {
     ...
     */
    private async awardAchievement(userId: string, code: string): Promise<AchievementWithEarned | null> {
        ...
        if (achievement.xpReward > 0) {
            await this.xpStreak.awardXp(userId, achievement.xpReward, `achievement_${code}`);
        }
