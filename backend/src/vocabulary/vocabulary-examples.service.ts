...
export class VocabularyExamplesService {
    ...
    private saveToCache(hanzi: string, examples: ExampleSentence[]): void {
        ...
        if (this.cache.size >= this.MAX_CACHE_SIZE) {
            ...
            console.log(`[ExamplesCache] Cleaned ${keysToDelete.length} oldest entries`);
        }
    ...
    async getExamples(vocabularyId: string): Promise<ExampleSentence[]> {
        ...
        const vocab = await this.prisma.vocabulary.findUnique({
            ...
            select: { id: true, hanzi: true, pinyin: true, meaningVi: true, meaningEn: true, examples: true },
        });
     ...
     */
    async getExamplesByHanzi(hanzi: string, pinyin?: string, meaning?: string): Promise<ExampleSentence[]> {
        // 1. Check in-memory cache first (fastest)
        const cached = this.getFromCache(hanzi);
        if (cached) {
            console.log(`[ExamplesCache] Hit for: ${hanzi}`);
            return cached;
        }
        console.log(`[ExamplesCache] Miss for: ${hanzi}`);
     ...
     */
    private async generateExamples(hanzi: string, pinyin: string, meaning: string): Promise<ExampleSentence[]> {
        const apiKey = process.env.GEMINI_API_KEY;

        console.log('[Gemini] Generating examples for:', hanzi);
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                try {
                    ...
                    console.log('[Gemini] Successfully parsed', examples.length, 'examples');
                    return examples.slice(0, 3);
                } catch (e) {
                    console.log('[Gemini] Attempting JSON repair for examples...');
                    ...
                    try {
                        ...
                        console.log('[Gemini] Repaired examples, got', repaired.length, 'items');
