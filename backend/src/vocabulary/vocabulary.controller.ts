...
export class VocabularyController {
  ...
  @ApiResponse({
    status: 200,
    description: 'List of vocabulary with pagination',
  })
  async findAll(
  ...
  @ApiOperation({
    summary:
      'Smart lookup - find by hanzi, breakdown characters, or fuzzy search',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Example sentences (from cache or AI-generated)',
  })
  ...
  @ApiResponse({
    status: 409,
    description: 'Vocabulary with this hanzi already exists',
  })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Delete all vocabulary (admin only, for re-import)',
  })
  ...
  @ApiOperation({
    summary:
      '[Admin] Validate vocabulary data before import to check for duplicates',
  })
  @ApiBody({
  ...
  @ApiResponse({
    status: 200,
    description: 'Validation result with duplicates',
  })
  ...
  @ApiOperation({
    summary:
      '[Admin] Import vocabulary from XLSX/CSV data with conflict resolution',
  })
  @ApiBody({
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: '[Admin] Bulk update existing vocabulary from XLSX/CSV data',
  })
  @ApiBody({
    description:
      'Array of vocabulary items to update (only existing items will be updated)',
    schema: {
      type: 'array',
      items: {
        type: 'object',
        ...
        properties: {
          hanzi: {
            type: 'string',
            example: '好',
            description: 'Required - used to find existing vocabulary',
          },
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        skipped: {
          type: 'number',
          description: 'Number of skipped items (not found)',
        },
