...
export class XpStreakService {
  private readonly logger = new Logger(XpStreakService.name);
  ...
  async awardXp(
    ...
    this.logger.debug(
      `Awarded ${amount} XP to user ${userId}. Reason: ${reason ?? 'n/a'}. New total: ${user.xp}`,
    );
    return user.xp;
  }
  ...
  async updateStreak(
    ...
    const vietnamNow = new Date(
      now.getTime() + now.getTimezoneOffset() * 60 * 1000 + vietnamOffset,
    );
