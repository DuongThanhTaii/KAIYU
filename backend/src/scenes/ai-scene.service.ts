...
export class AiSceneService {
  ...
  private async callWithRetry(
    ...
    if (response.status === 429 && retryCount < this.MAX_RETRIES) {
      ...
      this.logger.warn(
        `[AI Scene] Rate limited, retrying in ${delayMs / 1000}s... (attempt ${retryCount + 1}/${this.MAX_RETRIES})`,
      );
...
Return ONLY a valid JSON object with this exact structure (no markdown, no explanation):
{
  ...
  "end_success": {
    ...
    try {
      this.logger.debug(
        `[AI Scene] Generating scene for: ${request.scenarioType}`,
      );
      ...
      if (jsonMatch) {
        ...
        this.logger.debug(
          `[AI Scene] Successfully generated scene with ${Object.keys(dialogFlow).length} nodes`,
        );
        return dialogFlow;
      }
   ...
   */
  async getCachedScene(
    vocabularyUsed: VocabItem[],
  ): Promise<DialogFlow | null> {
    const vocabHash = this.hashVocab(vocabularyUsed);
