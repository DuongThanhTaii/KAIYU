...
export class ScenesController {
  ...
  async generateScene(@Body() dto: GenerateSceneDto, @Request() req: any) {
  ...
  async generateAiScene(@Body() dto: GenerateAiSceneDto, @Request() req: any) {
    const userId = req.user.userId;

    // Check for cached scene first
    const cached = await this.aiSceneService.getCachedScene(
      dto.vocabularyToUse,
    );
    if (cached) {
  ...
  async saveHistory(@Body() dto: SaveHistoryDto, @Request() req: any) {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Get vocabulary suggestions for a specific scenario',
  })
