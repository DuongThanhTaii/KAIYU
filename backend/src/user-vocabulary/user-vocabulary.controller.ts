...
export class UserVocabularyController {
    ...
    async saveVocabulary(
        @CurrentUser() user: any,
        @Body() body: { vocabularyId: string; sourceVideoId?: string; folderId?: string },
    ) {
        return this.userVocabularyService.saveVocabulary(user.id, body);
    }

    @Post('word')
    @ApiOperation({ summary: 'Save a word from dictionary lookup (creates vocabulary if needed)' })
    @ApiResponse({ status: 201, description: 'Word saved' })
    @ApiResponse({ status: 409, description: 'Already saved' })
    async saveWord(
        @CurrentUser() user: any,
        @Body() body: { hanzi: string; pinyin?: string; meaningVi?: string; sourceVideoId?: string; folderId?: string },
    ) {
